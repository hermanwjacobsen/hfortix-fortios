...
FIELD_TYPES = {
    ...
    "type": "option",  # Configure a static NAT server load balance VIP or access proxy.
    "src-filter": "string",  # Source IP6 filter (x:x:x:x:x:x:x:x/x). Separate addresses with spaces.
    "src-vip-filter": "option",  # Enable/disable use of 'src-filter' to match destinations for the reverse SNAT ru
    "extip": "user",  # IPv6 address or address range on the external interface that you want to map to 
    "mappedip": "user",  # Mapped IPv6 address range in the format startIP-endIP.
    "nat-source-vip": "option",  # Enable to perform SNAT on traffic from mappedip to the extip for all egress inte
    "ndp-reply": "option",  # Enable/disable this FortiGate unit's ability to respond to NDP requests for this
    ...
    "embedded-ipv4-address": "option",  # Enable/disable use of the lower 32 bits of the external IPv6 address as mapped I
    "ipv4-mappedip": "user",  # Range of mapped IP addresses. Specify the start IP address followed by a space a
    "ipv4-mappedport": "user",  # IPv4 port number range on the destination network to which the external port num
}
