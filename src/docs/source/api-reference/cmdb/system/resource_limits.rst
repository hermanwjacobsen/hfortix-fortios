ResourceLimits
==============

Configuration endpoint for system/resource_limits.

Python Attribute
----------------

.. code-block:: python

   fgt.api.cmdb.system.resource_limits

Available Methods
-----------------

- ``get()`` - GET operation
- ``put()`` - PUT operation

Examples
--------

.. code-block:: python

   from hfortix_fortios import FortiOS
   
   fgt = FortiOS(host='192.168.1.99', token='your-token')

   # List all items
   items = fgt.api.cmdb.system.resource_limits.get()


   # Update existing item
   result = fgt.api.cmdb.system.resource_limits.put(
       session='updated-value',
       ipsec_phase1='updated-value',
       ipsec_phase2='updated-value',
       ipsec_phase1_interface='updated-value',
   )


Method Reference
----------------

``get()``
^^^^^^^^^

.. code-block:: python

   get(
       payload_dict=None,
       exclude_default_values=None,
       stat_items=None,
       vdom=None,
       raw_json=False,
       **kwargs
   )

Select all entries in a CLI table.


``put()``
^^^^^^^^^

.. code-block:: python

   put(
       payload_dict=None,
       before=None,
       after=None,
       session=None,
       ipsec_phase1=None,
       ipsec_phase2=None,
       ipsec_phase1_interface=None,
       ipsec_phase2_interface=None,
       dialup_tunnel=None,
       firewall_policy=None,
       firewall_address=None,
       firewall_addrgrp=None,
       custom_service=None,
       service_group=None,
       onetime_schedule=None,
       # ... more parameters
   )

Update this specific resource.

