Radius
======

Configuration endpoint for user/radius.

Python Attribute
----------------

.. code-block:: python

   fgt.api.cmdb.user.radius

Available Methods
-----------------

- ``get()`` - GET operation
- ``post()`` - POST operation
- ``put()`` - PUT operation
- ``delete()`` - DELETE operation

Examples
--------

.. code-block:: python

   from hfortix_fortios import FortiOS
   
   fgt = FortiOS(host='192.168.1.99', token='your-token')

   # List all items
   items = fgt.api.cmdb.user.radius.get()

   # Get specific item by name
   item = fgt.api.cmdb.user.radius.get(name='item-name')

   # Create new item
   result = fgt.api.cmdb.user.radius.post(
       nkey='value',  # optional
       name='value',  # optional
       server='value',  # optional
   )

   # Update existing item
   result = fgt.api.cmdb.user.radius.put(
       name='updated-value',
       server='updated-value',
       secret='updated-value',
       secondary_server='updated-value',
   )

   # Delete item
   result = fgt.api.cmdb.user.radius.delete(name='item-name')


Method Reference
----------------

``get()``
^^^^^^^^^

.. code-block:: python

   get(
       name=None,
       payload_dict=None,
       attr=None,
       skip_to_datasource=None,
       acs=None,
       search=None,
       vdom=None,
       raw_json=False,
       **kwargs
   )

Select a specific entry from a CLI table.


``post()``
^^^^^^^^^^

.. code-block:: python

   post(
       payload_dict=None,
       nkey=None,
       name=None,
       server=None,
       secret=None,
       secondary_server=None,
       secondary_secret=None,
       tertiary_server=None,
       tertiary_secret=None,
       timeout=None,
       status_ttl=None,
       all_usergroup=None,
       use_management_vdom=None,
       switch_controller_nas_ip_dynamic=None,
       nas_ip=None,
       # ... more parameters
   )

Create object(s) in this table.


``put()``
^^^^^^^^^

.. code-block:: python

   put(
       name=None,
       payload_dict=None,
       before=None,
       after=None,
       server=None,
       secret=None,
       secondary_server=None,
       secondary_secret=None,
       tertiary_server=None,
       tertiary_secret=None,
       timeout=None,
       status_ttl=None,
       all_usergroup=None,
       use_management_vdom=None,
       switch_controller_nas_ip_dynamic=None,
       # ... more parameters
   )

Update this specific resource.


``delete()``
^^^^^^^^^^^^

.. code-block:: python

   delete(
       name=None,
       payload_dict=None,
       vdom=None,
       raw_json=False,
       **kwargs
   )

Delete this specific resource.

