...
class SyslogProfileObject(FortiObject[SyslogProfilePayload]):
    ...
    # Comment.
    comment: str
    # Enable/disable FortiAP units to send log messages to a syslog server (default = 
    server_status: Literal["enable", "disable"]
    # Syslog server CN domain name or IP address.
    server: str
    # Port number of syslog server that FortiAP units send log messages to (default = 
    server_port: int
    # Configure syslog server type (default = standard).
    server_type: Literal["standard", "fortianalyzer"]
    # Lowest level of log messages that FortiAP units send to this server (default = i
