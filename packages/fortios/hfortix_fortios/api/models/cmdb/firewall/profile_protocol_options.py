...
class ProfileProtocolOptionsHttp(BaseModel):
    ...
    http_0_9: Literal["allow", "block"] | None = Field(default="allow", description="Configure action to take upon receipt of HTTP 0.9 request.")
...
class ProfileProtocolOptionsModel(BaseModel):
    ...
    async def validate_cifs_references(self, client: Any) -> list[str]:
        ...
        Datasource endpoints checked:
        - user/domain_controller        - credential_store/domain_controller
        Args:
            client: FortiOS client instance (from fgt._client)
        ...
        for item in values:
            ...
            if not found:
                errors.append(
                    ...
                    "user/domain_controller or credential_store/domain_controller"
                )
        return errors
...
ProfileProtocolOptionsModelDict = dict[str, Any]  # For backward compatibility
