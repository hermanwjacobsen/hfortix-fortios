...
class GlobalModel(BaseModel):
    ...
    Validation Rules:
        - mac_aging_interval: min=10 max=1000000 pattern=
        - https_image_push: pattern=
        - vlan_all_mode: pattern=
        - vlan_optimization: pattern=
        - vlan_identity: pattern=
        - disable_discovery: pattern=
        - mac_retention_period: min=0 max=168 pattern=
        - default_virtual_switch_vlan: max_length=15 pattern=
        - dhcp_server_access_list: pattern=
        - dhcp_option82_format: pattern=
        - dhcp_option82_circuit_id: pattern=
        - dhcp_option82_remote_id: pattern=
        - dhcp_snoop_client_req: pattern=
        - dhcp_snoop_client_db_exp: min=300 max=259200 pattern=
        - dhcp_snoop_db_per_port_learn_limit: min=0 max=2048 pattern=
        - log_mac_limit_violations: pattern=
        - mac_violation_timer: min=0 max=4294967295 pattern=
        - sn_dns_resolution: pattern=
        - mac_event_logging: pattern=
        - bounce_quarantined_link: pattern=
        - quarantine_mode: pattern=
        - update_user_device: pattern=
        - custom_command: pattern=
        - fips_enforce: pattern=
        - firmware_provision_on_authorization: pattern=
        - switch_on_deauth: pattern=
        - firewall_auth_user_hold_period: min=5 max=1440 pattern=
    """
    ...
    disable_discovery: list[GlobalDisableDiscovery] | None = Field(default=None, description="Prevent this FortiSwitch from discovering.")
    ...
    dhcp_option82_circuit_id: str | GlobalDhcpOption82CircuitIdEnum | None = Field(default=None, description="List the parameters to be included to inform about client identification.")
    dhcp_option82_remote_id: Literal["mac", "hostname", "ip"] | None = Field(default="mac", description="List the parameters to be included to inform about client identification.")
    ...
    update_user_device: str | GlobalUpdateUserDeviceEnum | None = Field(default=None, description="Control which sources update the device user list.")
    custom_command: list[GlobalCustomCommand] | None = Field(default=None, description="List of custom commands to be pushed to all FortiSwitches in the VDOM.")
    ...
    async def validate_custom_command_references(self, client: Any) -> list[str]:
        ...
        Datasource endpoints checked:
        - switch_controller/custom_command
        Args:
            client: FortiOS client instance (from fgt._client)
...
# Schema: 1.7.0
# Generated: 2026-01-14T22:43:34.428014Z
# ============================================================================
