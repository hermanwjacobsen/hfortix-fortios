...
class WtpProfileRadio1(BaseModel):
    ...
    x80211d: Literal["enable", "disable"] | None = Field(default="enable", description="Enable/disable 802.11d countryie(default = enable).")
    x80211mc: Literal["enable", "disable"] | None = Field(default="disable", description="Enable/disable 802.11mc responder mode (default = disable).")
...
class WtpProfileModel(BaseModel):
    ...
    def validate_apcfg_auto_cert_scep_https_ca(cls, v: Any) -> Any:
        ...
        # Additional datasource validation could be added here
        return v
    @field_validator('admin_auth_tacacs_plus')
    @classmethod
    def validate_admin_auth_tacacs_plus(cls, v: Any) -> Any:
        """
        Validate admin_auth_tacacs_plus field.
    ...
    async def validate_ble_profile_references(self, client: Any) -> list[str]:
        ...
        Datasource endpoints checked:
        - wireless_controller/ble_profile
        Args:
            client: FortiOS client instance (from fgt._client)
    ...
    async def validate_lw_profile_references(self, client: Any) -> list[str]:
        ...
        Datasource endpoints checked:
        - wireless_controller/lw_profile
        Args:
            client: FortiOS client instance (from fgt._client)
    ...
    async def validate_syslog_profile_references(self, client: Any) -> list[str]:
        ...
        Datasource endpoints checked:
        - wireless_controller/syslog_profile
        Args:
            client: FortiOS client instance (from fgt._client)
    ...
    async def validate_radio_1_references(self, client: Any) -> list[str]:
        ...
        Datasource endpoints checked:
        - wireless_controller/arrp_profile
        Args:
            client: FortiOS client instance (from fgt._client)
    ...
    async def validate_apcfg_auto_cert_scep_https_ca_references(self, client: Any) -> list[str]:
        ...
        return errors
    async def validate_admin_auth_tacacs_plus_references(self, client: Any) -> list[str]:
        """
        Validate admin_auth_tacacs_plus references exist in FortiGate.
        ...
        Example:
            ...
            >>> fgt = FortiOS(host="192.168.1.1", token="your-token")
            >>> policy = WtpProfileModel(
            ...     admin_auth_tacacs_plus="invalid-name",
            ... )
            >>>
            >>> # Validate before posting
            >>> errors = await policy.validate_admin_auth_tacacs_plus_references(fgt._client)
            >>> if errors:
            ...     print("Validation failed:", errors)
            ... else:
            ...     result = await fgt.api.cmdb.wireless_controller.wtp_profile.post(policy.to_fortios_dict())
        ...
        # Check all datasource endpoints
        found = False
        if await client.api.cmdb.user.tacacs_plus.exists(value):
            found = True
...
WtpProfileModelDict = dict[str, Any]  # For backward compatibility
