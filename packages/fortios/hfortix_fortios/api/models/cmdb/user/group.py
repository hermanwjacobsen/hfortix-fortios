...
class GroupModel(BaseModel):
    ...
    Validation Rules:
        ...
        - group_type: pattern=
        - authtimeout: min=0 max=43200 pattern=
        - auth_concurrent_override: pattern=
        - auth_concurrent_value: min=0 max=100 pattern=
        - http_digest_realm: max_length=35 pattern=
        - sso_attribute_value: max_length=511 pattern=
        - member: pattern=
        - match: pattern=
        - user_id: pattern=
        - password: pattern=
        ...
        - company: pattern=
        - email: pattern=
        - mobile_phone: pattern=
        - sms_server: pattern=
        - sms_custom_server: max_length=35 pattern=
        - expire_type: pattern=
        - expire: min=1 max=31536000 pattern=
        - max_accounts: min=0 max=500 pattern=
        - multiple_guest_add: pattern=
        - guest: pattern=
    """
    ...
    group_type: str | GroupGroupTypeEnum | None = Field(default="firewall", description="Set the group to be for firewall authentication, FSSO, RSSO, or guest users.")
    ...
    member: list[GroupMember] | None = Field(default=None, description="Names of users, peers, LDAP severs, RADIUS servers or external idp servers to add to the user group.")
    match: list[GroupMatch] | None = Field(default=None, description="Group matches.")
    ...
    guest: list[GroupGuest] | None = Field(default=None, description="Guest User.")
    # ========================================================================
    # Custom Validators
    # ========================================================================
    ...
    async def validate_member_references(self, client: Any) -> list[str]:
        ...
        - user/peer        - user/local        - user/radius        - user/tacacs_plus        - user/ldap        - user/saml        - user/external_identity_provider        - user/adgrp        - user/pop3        - user/certificate
        Args:
        ...
        for item in values:
            ...
            if not found:
                errors.append(
                    ...
                    "user/peer or user/local or user/radius or user/tacacs_plus or user/ldap or user/saml or user/external_identity_provider or user/adgrp or user/pop3 or user/certificate"
                )
        return errors
    async def validate_match_references(self, client: Any) -> list[str]:
        ...
        Datasource endpoints checked:
        - user/radius        - user/ldap        - user/tacacs_plus        - user/saml        - user/external_identity_provider
        Args:
            client: FortiOS client instance (from fgt._client)
        ...
        for item in values:
            ...
            if not found:
                errors.append(
                    ...
                    "user/radius or user/ldap or user/tacacs_plus or user/saml or user/external_identity_provider"
                )
        return errors
...
# Schema: 1.7.0
# Generated: 2026-01-14T22:43:39.061115Z
# ============================================================================
