...
def validate_webfilter_search_engine_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_webfilter_search_engine_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_webfilter_search_engine_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for webfilter/search_engine.
...
def validate_webfilter_search_engine_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["safesearch"] = "invalid-value"
        >>> is_valid, error = validate_webfilter_search_engine_post(payload)
    ...
    if "safesearch" in payload:
        value = payload["safesearch"]
        if value not in VALID_BODY_SAFESEARCH:
            desc = FIELD_DESCRIPTIONS.get("safesearch", "")
            error_msg = f"Invalid value for 'safesearch': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_SAFESEARCH)}"
            error_msg += f"\n  → Example: safesearch='{{ VALID_BODY_SAFESEARCH[0] }}'"
            return (False, error_msg)
    if "charset" in payload:
        value = payload["charset"]
        if value not in VALID_BODY_CHARSET:
            desc = FIELD_DESCRIPTIONS.get("charset", "")
            error_msg = f"Invalid value for 'charset': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_CHARSET)}"
            error_msg += f"\n  → Example: charset='{{ VALID_BODY_CHARSET[0] }}'"
            return (False, error_msg)
...
def validate_webfilter_search_engine_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_webfilter_search_engine_put(payload)
    """
    ...
    if "safesearch" in payload:
        ...
        if value not in VALID_BODY_SAFESEARCH:
            return (
                False,
                f"Invalid value for 'safesearch'='{value}'. Must be one of: {', '.join(VALID_BODY_SAFESEARCH)}",
            )
    if "charset" in payload:
        value = payload["charset"]
        if value not in VALID_BODY_CHARSET:
            return (
                False,
                f"Invalid value for 'charset'='{value}'. Must be one of: {', '.join(VALID_BODY_CHARSET)}",
            )
