"""
Validation helpers for web_proxy/url_match endpoint.
...
def validate_web_proxy_url_match_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_web_proxy_url_match_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_web_proxy_url_match_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for web_proxy/url_match.
...
def validate_web_proxy_url_match_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload = {
        ...     "url-pattern": True,  # URL pattern to be exempted from web proxy forwardi
        ... }
    ...
    if "cache-exemption" in payload:
        value = payload["cache-exemption"]
        if value not in VALID_BODY_CACHE_EXEMPTION:
            desc = FIELD_DESCRIPTIONS.get("cache-exemption", "")
            error_msg = f"Invalid value for 'cache-exemption': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_CACHE_EXEMPTION)}"
            error_msg += f"\n  → Example: cache-exemption='{{ VALID_BODY_CACHE_EXEMPTION[0] }}'"
            return (False, error_msg)
...
def validate_web_proxy_url_match_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_web_proxy_url_match_put(payload)
    """
    ...
    if "cache-exemption" in payload:
        ...
        if value not in VALID_BODY_CACHE_EXEMPTION:
            return (
                False,
                f"Invalid value for 'cache-exemption'='{value}'. Must be one of: {', '.join(VALID_BODY_CACHE_EXEMPTION)}",
            )
