"""
Validation helpers for log/fortianalyzer_cloud_override_filter endpoint.
...
def validate_log_fortianalyzer_cloud_override_filter_get(
    ...
    **params: Any,
    ...
    """
    Validate GET request parameters for log/fortianalyzer_cloud_override_filter.
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_log_fortianalyzer_cloud_override_filter_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for log/fortianalyzer_cloud_override_filter.
...
def validate_log_fortianalyzer_cloud_override_filter_post(
    ...
    **params: Any,
    ...
    """
    Validate POST request to create new log/fortianalyzer_cloud_override_filter object.
    ...
    Examples:
        ...
        >>> payload = {
        ... }
        >>> is_valid, error = validate_log_fortianalyzer_cloud_override_filter_post(payload)
        >>> assert is_valid == True
    ...
    if "dlp-archive" in payload:
        value = payload["dlp-archive"]
        if value not in VALID_BODY_DLP_ARCHIVE:
            desc = FIELD_DESCRIPTIONS.get("dlp-archive", "")
            error_msg = f"Invalid value for 'dlp-archive': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_DLP_ARCHIVE)}"
            error_msg += f"\n  → Example: dlp-archive='{{ VALID_BODY_DLP_ARCHIVE[0] }}'"
            return (False, error_msg)
    if "gtp" in payload:
        value = payload["gtp"]
...
def validate_log_fortianalyzer_cloud_override_filter_put(
    ...
    **params: Any,
    ...
    """
    Validate PUT request to update log/fortianalyzer_cloud_override_filter.
    ...
    Example:
        ...
        >>> is_valid, error = validate_log_fortianalyzer_cloud_override_filter_put(payload)
    """
    ...
    if "dlp-archive" in payload:
        ...
        if value not in VALID_BODY_DLP_ARCHIVE:
            return (
                False,
                f"Invalid value for 'dlp-archive'='{value}'. Must be one of: {', '.join(VALID_BODY_DLP_ARCHIVE)}",
            )
    if "gtp" in payload:
        value = payload["gtp"]
        if value not in VALID_BODY_GTP:
...
SCHEMA_INFO = {
    "endpoint": "log/fortianalyzer_cloud_override_filter",
    "category": "cmdb",
