...
def validate_report_setting_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_report_setting_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for report/setting.
...
def validate_report_setting_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["pdf-report"] = "invalid-value"
        >>> is_valid, error = validate_report_setting_post(payload)
    ...
    if "pdf-report" in payload:
        value = payload["pdf-report"]
        if value not in VALID_BODY_PDF_REPORT:
            desc = FIELD_DESCRIPTIONS.get("pdf-report", "")
            error_msg = f"Invalid value for 'pdf-report': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_PDF_REPORT)}"
            error_msg += f"\n  → Example: pdf-report='{{ VALID_BODY_PDF_REPORT[0] }}'"
            return (False, error_msg)
    if "fortiview" in payload:
        value = payload["fortiview"]
        if value not in VALID_BODY_FORTIVIEW:
            desc = FIELD_DESCRIPTIONS.get("fortiview", "")
            error_msg = f"Invalid value for 'fortiview': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_FORTIVIEW)}"
            error_msg += f"\n  → Example: fortiview='{{ VALID_BODY_FORTIVIEW[0] }}'"
            return (False, error_msg)
    if "report-source" in payload:
        value = payload["report-source"]
        if value not in VALID_BODY_REPORT_SOURCE:
            desc = FIELD_DESCRIPTIONS.get("report-source", "")
            error_msg = f"Invalid value for 'report-source': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_REPORT_SOURCE)}"
            error_msg += f"\n  → Example: report-source='{{ VALID_BODY_REPORT_SOURCE[0] }}'"
            return (False, error_msg)
...
def validate_report_setting_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_report_setting_put(payload)
    """
    ...
    if "pdf-report" in payload:
        ...
        if value not in VALID_BODY_PDF_REPORT:
            return (
                False,
                f"Invalid value for 'pdf-report'='{value}'. Must be one of: {', '.join(VALID_BODY_PDF_REPORT)}",
            )
    if "fortiview" in payload:
        value = payload["fortiview"]
        if value not in VALID_BODY_FORTIVIEW:
            return (
                False,
                f"Invalid value for 'fortiview'='{value}'. Must be one of: {', '.join(VALID_BODY_FORTIVIEW)}",
            )
    if "report-source" in payload:
        value = payload["report-source"]
        if value not in VALID_BODY_REPORT_SOURCE:
            return (
                False,
                f"Invalid value for 'report-source'='{value}'. Must be one of: {', '.join(VALID_BODY_REPORT_SOURCE)}",
            )
