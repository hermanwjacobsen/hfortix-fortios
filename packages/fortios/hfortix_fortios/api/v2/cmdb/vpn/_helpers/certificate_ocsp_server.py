...
def validate_vpn_certificate_ocsp_server_get(
    ...
    **params: Any,
    ...
    """
    Validate GET request parameters for vpn/certificate_ocsp_server.
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_vpn_certificate_ocsp_server_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_vpn_certificate_ocsp_server_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for vpn/certificate_ocsp_server.
...
def validate_vpn_certificate_ocsp_server_post(
    ...
    **params: Any,
    ...
    """
    Validate POST request to create new vpn/certificate_ocsp_server object.
    ...
    Examples:
        ...
        >>> payload["unavail-action"] = "invalid-value"
        >>> is_valid, error = validate_vpn_certificate_ocsp_server_post(payload)
    ...
    if "unavail-action" in payload:
        value = payload["unavail-action"]
        if value not in VALID_BODY_UNAVAIL_ACTION:
            desc = FIELD_DESCRIPTIONS.get("unavail-action", "")
            error_msg = f"Invalid value for 'unavail-action': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_UNAVAIL_ACTION)}"
            error_msg += f"\n  → Example: unavail-action='{{ VALID_BODY_UNAVAIL_ACTION[0] }}'"
...
def validate_vpn_certificate_ocsp_server_put(
    ...
    **params: Any,
    ...
    """
    Validate PUT request to update vpn/certificate_ocsp_server.
    ...
    Example:
        ...
        >>> is_valid, error = validate_vpn_certificate_ocsp_server_put(payload)
    """
    ...
    if "unavail-action" in payload:
        ...
        if value not in VALID_BODY_UNAVAIL_ACTION:
            return (
                False,
                f"Invalid value for 'unavail-action'='{value}'. Must be one of: {', '.join(VALID_BODY_UNAVAIL_ACTION)}",
            )
...
SCHEMA_INFO = {
    "endpoint": "vpn/certificate_ocsp_server",
    "category": "cmdb",
