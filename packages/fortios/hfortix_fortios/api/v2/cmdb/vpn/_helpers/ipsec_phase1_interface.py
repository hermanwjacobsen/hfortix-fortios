...
def validate_vpn_ipsec_phase1_interface_get(
    ...
    **params: Any,
    ...
    """
    Validate GET request parameters for vpn/ipsec_phase1_interface.
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_vpn_ipsec_phase1_interface_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_vpn_ipsec_phase1_interface_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for vpn/ipsec_phase1_interface.
...
def validate_vpn_ipsec_phase1_interface_post(
    ...
    **params: Any,
    ...
    """
    Validate POST request to create new vpn/ipsec_phase1_interface object.
    ...
    if "monitor-hold-down-type" in payload:
        value = payload["monitor-hold-down-type"]
        if value not in VALID_BODY_MONITOR_HOLD_DOWN_TYPE:
            desc = FIELD_DESCRIPTIONS.get("monitor-hold-down-type", "")
            error_msg = f"Invalid value for 'monitor-hold-down-type': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_MONITOR_HOLD_DOWN_TYPE)}"
            error_msg += f"\n  → Example: monitor-hold-down-type='{{ VALID_BODY_MONITOR_HOLD_DOWN_TYPE[0] }}'"
            return (False, error_msg)
    if "monitor-hold-down-weekday" in payload:
        value = payload["monitor-hold-down-weekday"]
        if value not in VALID_BODY_MONITOR_HOLD_DOWN_WEEKDAY:
            desc = FIELD_DESCRIPTIONS.get("monitor-hold-down-weekday", "")
            error_msg = f"Invalid value for 'monitor-hold-down-weekday': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_MONITOR_HOLD_DOWN_WEEKDAY)}"
            error_msg += f"\n  → Example: monitor-hold-down-weekday='{{ VALID_BODY_MONITOR_HOLD_DOWN_WEEKDAY[0] }}'"
            return (False, error_msg)
    if "net-device" in payload:
        value = payload["net-device"]
        if value not in VALID_BODY_NET_DEVICE:
            desc = FIELD_DESCRIPTIONS.get("net-device", "")
            error_msg = f"Invalid value for 'net-device': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_NET_DEVICE)}"
            error_msg += f"\n  → Example: net-device='{{ VALID_BODY_NET_DEVICE[0] }}'"
            return (False, error_msg)
    if "passive-mode" in payload:
        value = payload["passive-mode"]
        if value not in VALID_BODY_PASSIVE_MODE:
            desc = FIELD_DESCRIPTIONS.get("passive-mode", "")
            error_msg = f"Invalid value for 'passive-mode': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_PASSIVE_MODE)}"
            error_msg += f"\n  → Example: passive-mode='{{ VALID_BODY_PASSIVE_MODE[0] }}'"
            return (False, error_msg)
    if "exchange-interface-ip" in payload:
        value = payload["exchange-interface-ip"]
        if value not in VALID_BODY_EXCHANGE_INTERFACE_IP:
            desc = FIELD_DESCRIPTIONS.get("exchange-interface-ip", "")
            error_msg = f"Invalid value for 'exchange-interface-ip': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_EXCHANGE_INTERFACE_IP)}"
            error_msg += f"\n  → Example: exchange-interface-ip='{{ VALID_BODY_EXCHANGE_INTERFACE_IP[0] }}'"
            return (False, error_msg)
    if "aggregate-member" in payload:
        value = payload["aggregate-member"]
        if value not in VALID_BODY_AGGREGATE_MEMBER:
            desc = FIELD_DESCRIPTIONS.get("aggregate-member", "")
            error_msg = f"Invalid value for 'aggregate-member': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AGGREGATE_MEMBER)}"
            error_msg += f"\n  → Example: aggregate-member='{{ VALID_BODY_AGGREGATE_MEMBER[0] }}'"
            return (False, error_msg)
    if "packet-redistribution" in payload:
        value = payload["packet-redistribution"]
        if value not in VALID_BODY_PACKET_REDISTRIBUTION:
            desc = FIELD_DESCRIPTIONS.get("packet-redistribution", "")
            error_msg = f"Invalid value for 'packet-redistribution': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_PACKET_REDISTRIBUTION)}"
            error_msg += f"\n  → Example: packet-redistribution='{{ VALID_BODY_PACKET_REDISTRIBUTION[0] }}'"
            return (False, error_msg)
    if "peer-egress-shaping" in payload:
        value = payload["peer-egress-shaping"]
        if value not in VALID_BODY_PEER_EGRESS_SHAPING:
            desc = FIELD_DESCRIPTIONS.get("peer-egress-shaping", "")
            error_msg = f"Invalid value for 'peer-egress-shaping': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_PEER_EGRESS_SHAPING)}"
            error_msg += f"\n  → Example: peer-egress-shaping='{{ VALID_BODY_PEER_EGRESS_SHAPING[0] }}'"
            return (False, error_msg)
    ...
    if "ip-fragmentation" in payload:
        value = payload["ip-fragmentation"]
        if value not in VALID_BODY_IP_FRAGMENTATION:
            desc = FIELD_DESCRIPTIONS.get("ip-fragmentation", "")
            error_msg = f"Invalid value for 'ip-fragmentation': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_IP_FRAGMENTATION)}"
            error_msg += f"\n  → Example: ip-fragmentation='{{ VALID_BODY_IP_FRAGMENTATION[0] }}'"
            return (False, error_msg)
    if "dpd" in payload:
        value = payload["dpd"]
    ...
    if "auto-discovery-sender" in payload:
        value = payload["auto-discovery-sender"]
        if value not in VALID_BODY_AUTO_DISCOVERY_SENDER:
            desc = FIELD_DESCRIPTIONS.get("auto-discovery-sender", "")
            error_msg = f"Invalid value for 'auto-discovery-sender': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AUTO_DISCOVERY_SENDER)}"
            error_msg += f"\n  → Example: auto-discovery-sender='{{ VALID_BODY_AUTO_DISCOVERY_SENDER[0] }}'"
            return (False, error_msg)
    if "auto-discovery-receiver" in payload:
        value = payload["auto-discovery-receiver"]
        if value not in VALID_BODY_AUTO_DISCOVERY_RECEIVER:
            desc = FIELD_DESCRIPTIONS.get("auto-discovery-receiver", "")
            error_msg = f"Invalid value for 'auto-discovery-receiver': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AUTO_DISCOVERY_RECEIVER)}"
            error_msg += f"\n  → Example: auto-discovery-receiver='{{ VALID_BODY_AUTO_DISCOVERY_RECEIVER[0] }}'"
            return (False, error_msg)
    if "auto-discovery-forwarder" in payload:
        value = payload["auto-discovery-forwarder"]
        if value not in VALID_BODY_AUTO_DISCOVERY_FORWARDER:
            desc = FIELD_DESCRIPTIONS.get("auto-discovery-forwarder", "")
            error_msg = f"Invalid value for 'auto-discovery-forwarder': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AUTO_DISCOVERY_FORWARDER)}"
            error_msg += f"\n  → Example: auto-discovery-forwarder='{{ VALID_BODY_AUTO_DISCOVERY_FORWARDER[0] }}'"
            return (False, error_msg)
    if "auto-discovery-psk" in payload:
        value = payload["auto-discovery-psk"]
        if value not in VALID_BODY_AUTO_DISCOVERY_PSK:
            desc = FIELD_DESCRIPTIONS.get("auto-discovery-psk", "")
            error_msg = f"Invalid value for 'auto-discovery-psk': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AUTO_DISCOVERY_PSK)}"
            error_msg += f"\n  → Example: auto-discovery-psk='{{ VALID_BODY_AUTO_DISCOVERY_PSK[0] }}'"
            return (False, error_msg)
    if "auto-discovery-shortcuts" in payload:
        value = payload["auto-discovery-shortcuts"]
        if value not in VALID_BODY_AUTO_DISCOVERY_SHORTCUTS:
            desc = FIELD_DESCRIPTIONS.get("auto-discovery-shortcuts", "")
            error_msg = f"Invalid value for 'auto-discovery-shortcuts': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AUTO_DISCOVERY_SHORTCUTS)}"
            error_msg += f"\n  → Example: auto-discovery-shortcuts='{{ VALID_BODY_AUTO_DISCOVERY_SHORTCUTS[0] }}'"
            return (False, error_msg)
    if "auto-discovery-crossover" in payload:
        value = payload["auto-discovery-crossover"]
        if value not in VALID_BODY_AUTO_DISCOVERY_CROSSOVER:
            desc = FIELD_DESCRIPTIONS.get("auto-discovery-crossover", "")
            error_msg = f"Invalid value for 'auto-discovery-crossover': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AUTO_DISCOVERY_CROSSOVER)}"
            error_msg += f"\n  → Example: auto-discovery-crossover='{{ VALID_BODY_AUTO_DISCOVERY_CROSSOVER[0] }}'"
            return (False, error_msg)
    if "auto-discovery-dialup-placeholder" in payload:
        value = payload["auto-discovery-dialup-placeholder"]
        if value not in VALID_BODY_AUTO_DISCOVERY_DIALUP_PLACEHOLDER:
            desc = FIELD_DESCRIPTIONS.get("auto-discovery-dialup-placeholder", "")
            error_msg = f"Invalid value for 'auto-discovery-dialup-placeholder': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AUTO_DISCOVERY_DIALUP_PLACEHOLDER)}"
            error_msg += f"\n  → Example: auto-discovery-dialup-placeholder='{{ VALID_BODY_AUTO_DISCOVERY_DIALUP_PLACEHOLDER[0] }}'"
            return (False, error_msg)
    ...
    if "encapsulation-address" in payload:
        value = payload["encapsulation-address"]
        if value not in VALID_BODY_ENCAPSULATION_ADDRESS:
            desc = FIELD_DESCRIPTIONS.get("encapsulation-address", "")
            error_msg = f"Invalid value for 'encapsulation-address': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_ENCAPSULATION_ADDRESS)}"
            error_msg += f"\n  → Example: encapsulation-address='{{ VALID_BODY_ENCAPSULATION_ADDRESS[0] }}'"
            return (False, error_msg)
...
def validate_vpn_ipsec_phase1_interface_put(
    ...
    **params: Any,
    ...
    """
    Validate PUT request to update vpn/ipsec_phase1_interface.
    ...
    Example:
        ...
        >>> is_valid, error = validate_vpn_ipsec_phase1_interface_put(payload)
    """
    ...
    if "monitor-hold-down-type" in payload:
        ...
        if value not in VALID_BODY_MONITOR_HOLD_DOWN_TYPE:
            return (
                False,
                f"Invalid value for 'monitor-hold-down-type'='{value}'. Must be one of: {', '.join(VALID_BODY_MONITOR_HOLD_DOWN_TYPE)}",
            )
    if "monitor-hold-down-weekday" in payload:
        value = payload["monitor-hold-down-weekday"]
        if value not in VALID_BODY_MONITOR_HOLD_DOWN_WEEKDAY:
            return (
                False,
                f"Invalid value for 'monitor-hold-down-weekday'='{value}'. Must be one of: {', '.join(VALID_BODY_MONITOR_HOLD_DOWN_WEEKDAY)}",
            )
    if "net-device" in payload:
        value = payload["net-device"]
        if value not in VALID_BODY_NET_DEVICE:
            return (
                False,
                f"Invalid value for 'net-device'='{value}'. Must be one of: {', '.join(VALID_BODY_NET_DEVICE)}",
            )
    if "passive-mode" in payload:
        value = payload["passive-mode"]
        if value not in VALID_BODY_PASSIVE_MODE:
            return (
                False,
                f"Invalid value for 'passive-mode'='{value}'. Must be one of: {', '.join(VALID_BODY_PASSIVE_MODE)}",
            )
    if "exchange-interface-ip" in payload:
        value = payload["exchange-interface-ip"]
        if value not in VALID_BODY_EXCHANGE_INTERFACE_IP:
            return (
                False,
                f"Invalid value for 'exchange-interface-ip'='{value}'. Must be one of: {', '.join(VALID_BODY_EXCHANGE_INTERFACE_IP)}",
            )
    if "aggregate-member" in payload:
        value = payload["aggregate-member"]
        if value not in VALID_BODY_AGGREGATE_MEMBER:
            return (
                False,
                f"Invalid value for 'aggregate-member'='{value}'. Must be one of: {', '.join(VALID_BODY_AGGREGATE_MEMBER)}",
            )
    if "packet-redistribution" in payload:
        value = payload["packet-redistribution"]
        if value not in VALID_BODY_PACKET_REDISTRIBUTION:
            return (
                False,
                f"Invalid value for 'packet-redistribution'='{value}'. Must be one of: {', '.join(VALID_BODY_PACKET_REDISTRIBUTION)}",
            )
    if "peer-egress-shaping" in payload:
        value = payload["peer-egress-shaping"]
        if value not in VALID_BODY_PEER_EGRESS_SHAPING:
            return (
                False,
                f"Invalid value for 'peer-egress-shaping'='{value}'. Must be one of: {', '.join(VALID_BODY_PEER_EGRESS_SHAPING)}",
            )
    if "mode-cfg" in payload:
        value = payload["mode-cfg"]
        if value not in VALID_BODY_MODE_CFG:
    ...
    if "ip-fragmentation" in payload:
        ...
        if value not in VALID_BODY_IP_FRAGMENTATION:
            return (
                False,
                f"Invalid value for 'ip-fragmentation'='{value}'. Must be one of: {', '.join(VALID_BODY_IP_FRAGMENTATION)}",
            )
    if "dpd" in payload:
        value = payload["dpd"]
        if value not in VALID_BODY_DPD:
    ...
    if "auto-discovery-sender" in payload:
        ...
        if value not in VALID_BODY_AUTO_DISCOVERY_SENDER:
            return (
                False,
                f"Invalid value for 'auto-discovery-sender'='{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_DISCOVERY_SENDER)}",
            )
    if "auto-discovery-receiver" in payload:
        value = payload["auto-discovery-receiver"]
        if value not in VALID_BODY_AUTO_DISCOVERY_RECEIVER:
            return (
                False,
                f"Invalid value for 'auto-discovery-receiver'='{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_DISCOVERY_RECEIVER)}",
            )
    if "auto-discovery-forwarder" in payload:
        value = payload["auto-discovery-forwarder"]
        if value not in VALID_BODY_AUTO_DISCOVERY_FORWARDER:
            return (
                False,
                f"Invalid value for 'auto-discovery-forwarder'='{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_DISCOVERY_FORWARDER)}",
            )
    if "auto-discovery-psk" in payload:
        value = payload["auto-discovery-psk"]
        if value not in VALID_BODY_AUTO_DISCOVERY_PSK:
            return (
                False,
                f"Invalid value for 'auto-discovery-psk'='{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_DISCOVERY_PSK)}",
            )
    if "auto-discovery-shortcuts" in payload:
        value = payload["auto-discovery-shortcuts"]
        if value not in VALID_BODY_AUTO_DISCOVERY_SHORTCUTS:
            return (
                False,
                f"Invalid value for 'auto-discovery-shortcuts'='{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_DISCOVERY_SHORTCUTS)}",
            )
    if "auto-discovery-crossover" in payload:
        value = payload["auto-discovery-crossover"]
        if value not in VALID_BODY_AUTO_DISCOVERY_CROSSOVER:
            return (
                False,
                f"Invalid value for 'auto-discovery-crossover'='{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_DISCOVERY_CROSSOVER)}",
            )
    if "auto-discovery-dialup-placeholder" in payload:
        value = payload["auto-discovery-dialup-placeholder"]
        if value not in VALID_BODY_AUTO_DISCOVERY_DIALUP_PLACEHOLDER:
            return (
                False,
                f"Invalid value for 'auto-discovery-dialup-placeholder'='{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_DISCOVERY_DIALUP_PLACEHOLDER)}",
            )
    if "encapsulation" in payload:
        value = payload["encapsulation"]
        if value not in VALID_BODY_ENCAPSULATION:
    ...
    if "encapsulation-address" in payload:
        ...
        if value not in VALID_BODY_ENCAPSULATION_ADDRESS:
            return (
                False,
                f"Invalid value for 'encapsulation-address'='{value}'. Must be one of: {', '.join(VALID_BODY_ENCAPSULATION_ADDRESS)}",
            )
    if "nattraversal" in payload:
        value = payload["nattraversal"]
        if value not in VALID_BODY_NATTRAVERSAL:
...
SCHEMA_INFO = {
    "endpoint": "vpn/ipsec_phase1_interface",
    "category": "cmdb",
