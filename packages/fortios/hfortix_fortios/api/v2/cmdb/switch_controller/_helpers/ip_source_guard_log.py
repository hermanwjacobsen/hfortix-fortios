"""
Validation helpers for switch_controller/ip_source_guard_log endpoint.
...
def validate_switch_controller_ip_source_guard_log_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_switch_controller_ip_source_guard_log_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for switch_controller/ip_source_guard_log.
...
def validate_switch_controller_ip_source_guard_log_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["log-violations"] = "invalid-value"
        >>> is_valid, error = validate_switch_controller_ip_source_guard_log_post(payload)
    ...
    if "log-violations" in payload:
        value = payload["log-violations"]
        if value not in VALID_BODY_LOG_VIOLATIONS:
            desc = FIELD_DESCRIPTIONS.get("log-violations", "")
            error_msg = f"Invalid value for 'log-violations': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_LOG_VIOLATIONS)}"
            error_msg += f"\n  → Example: log-violations='{{ VALID_BODY_LOG_VIOLATIONS[0] }}'"
...
def validate_switch_controller_ip_source_guard_log_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_switch_controller_ip_source_guard_log_put(payload)
    """
    ...
    if "log-violations" in payload:
        ...
        if value not in VALID_BODY_LOG_VIOLATIONS:
            return (
                False,
                f"Invalid value for 'log-violations'='{value}'. Must be one of: {', '.join(VALID_BODY_LOG_VIOLATIONS)}",
            )
