"""
Validation helpers for switch_controller/traffic_policy endpoint.
...
def validate_switch_controller_traffic_policy_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_switch_controller_traffic_policy_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_switch_controller_traffic_policy_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for switch_controller/traffic_policy.
...
def validate_switch_controller_traffic_policy_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["policer-status"] = "invalid-value"
        >>> is_valid, error = validate_switch_controller_traffic_policy_post(payload)
    ...
    if "policer-status" in payload:
        value = payload["policer-status"]
        if value not in VALID_BODY_POLICER_STATUS:
            desc = FIELD_DESCRIPTIONS.get("policer-status", "")
            error_msg = f"Invalid value for 'policer-status': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_POLICER_STATUS)}"
            error_msg += f"\n  → Example: policer-status='{{ VALID_BODY_POLICER_STATUS[0] }}'"
...
def validate_switch_controller_traffic_policy_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_switch_controller_traffic_policy_put(payload)
    """
    ...
    if "policer-status" in payload:
        ...
        if value not in VALID_BODY_POLICER_STATUS:
            return (
                False,
                f"Invalid value for 'policer-status'='{value}'. Must be one of: {', '.join(VALID_BODY_POLICER_STATUS)}",
            )
    if "type" in payload:
        value = payload["type"]
        if value not in VALID_BODY_TYPE:
