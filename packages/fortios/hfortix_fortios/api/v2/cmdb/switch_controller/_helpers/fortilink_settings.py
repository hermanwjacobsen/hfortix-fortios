"""
Validation helpers for switch_controller/fortilink_settings endpoint.
...
def validate_switch_controller_fortilink_settings_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_switch_controller_fortilink_settings_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_switch_controller_fortilink_settings_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for switch_controller/fortilink_settings.
...
def validate_switch_controller_fortilink_settings_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["link-down-flush"] = "invalid-value"
        >>> is_valid, error = validate_switch_controller_fortilink_settings_post(payload)
    ...
    if "link-down-flush" in payload:
        value = payload["link-down-flush"]
        if value not in VALID_BODY_LINK_DOWN_FLUSH:
            desc = FIELD_DESCRIPTIONS.get("link-down-flush", "")
            error_msg = f"Invalid value for 'link-down-flush': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_LINK_DOWN_FLUSH)}"
            error_msg += f"\n  → Example: link-down-flush='{{ VALID_BODY_LINK_DOWN_FLUSH[0] }}'"
            return (False, error_msg)
    if "access-vlan-mode" in payload:
        value = payload["access-vlan-mode"]
        if value not in VALID_BODY_ACCESS_VLAN_MODE:
            desc = FIELD_DESCRIPTIONS.get("access-vlan-mode", "")
            error_msg = f"Invalid value for 'access-vlan-mode': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_ACCESS_VLAN_MODE)}"
            error_msg += f"\n  → Example: access-vlan-mode='{{ VALID_BODY_ACCESS_VLAN_MODE[0] }}'"
            return (False, error_msg)
...
def validate_switch_controller_fortilink_settings_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_switch_controller_fortilink_settings_put(payload)
    """
    ...
    if "link-down-flush" in payload:
        ...
        if value not in VALID_BODY_LINK_DOWN_FLUSH:
            return (
                False,
                f"Invalid value for 'link-down-flush'='{value}'. Must be one of: {', '.join(VALID_BODY_LINK_DOWN_FLUSH)}",
            )
    if "access-vlan-mode" in payload:
        value = payload["access-vlan-mode"]
        if value not in VALID_BODY_ACCESS_VLAN_MODE:
            return (
                False,
                f"Invalid value for 'access-vlan-mode'='{value}'. Must be one of: {', '.join(VALID_BODY_ACCESS_VLAN_MODE)}",
            )
