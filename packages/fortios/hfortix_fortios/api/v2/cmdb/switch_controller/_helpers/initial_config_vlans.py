...
def validate_switch_controller_initial_config_vlans_get(
    ...
    **params: Any,
    ...
    """
    Validate GET request parameters for switch-controller/initial_config_vlans.
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_switch_controller_initial_config_vlans_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for switch-controller/initial_config_vlans.
...
def validate_switch_controller_initial_config_vlans_post(
    ...
    **params: Any,
    ...
    """
    Validate POST request to create new switch-controller/initial_config_vlans object.
    ...
    Examples:
        ...
        >>> payload["optional-vlans"] = "invalid-value"
        >>> is_valid, error = validate_switch_controller_initial_config_vlans_post(payload)
    ...
    if "optional-vlans" in payload:
        value = payload["optional-vlans"]
        if value not in VALID_BODY_OPTIONAL_VLANS:
            desc = FIELD_DESCRIPTIONS.get("optional-vlans", "")
            error_msg = f"Invalid value for 'optional-vlans': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_OPTIONAL_VLANS)}"
            error_msg += f"\n  → Example: optional-vlans='{{ VALID_BODY_OPTIONAL_VLANS[0] }}'"
...
def validate_switch_controller_initial_config_vlans_put(
    ...
    **params: Any,
    ...
    """
    Validate PUT request to update switch-controller/initial_config_vlans.
    ...
    Example:
        ...
        >>> is_valid, error = validate_switch_controller_initial_config_vlans_put(payload)
    """
    ...
    if "optional-vlans" in payload:
        ...
        if value not in VALID_BODY_OPTIONAL_VLANS:
            return (
                False,
                f"Invalid value for 'optional-vlans'='{value}'. Must be one of: {', '.join(VALID_BODY_OPTIONAL_VLANS)}",
            )
...
SCHEMA_INFO = {
    "endpoint": "switch-controller/initial_config_vlans",
    "category": "cmdb",
