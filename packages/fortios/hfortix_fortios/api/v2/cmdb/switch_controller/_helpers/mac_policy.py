"""
Validation helpers for switch_controller/mac_policy endpoint.
...
def validate_switch_controller_mac_policy_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_switch_controller_mac_policy_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_switch_controller_mac_policy_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for switch_controller/mac_policy.
...
def validate_switch_controller_mac_policy_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload = {
        ...     "fortilink": True,  # FortiLink interface for which this MAC policy belo
        ... }
    ...
    if "count" in payload:
        value = payload["count"]
        if value not in VALID_BODY_COUNT:
            desc = FIELD_DESCRIPTIONS.get("count", "")
            error_msg = f"Invalid value for 'count': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_COUNT)}"
            error_msg += f"\n  → Example: count='{{ VALID_BODY_COUNT[0] }}'"
            return (False, error_msg)
    if "bounce-port-link" in payload:
        value = payload["bounce-port-link"]
        if value not in VALID_BODY_BOUNCE_PORT_LINK:
            desc = FIELD_DESCRIPTIONS.get("bounce-port-link", "")
            error_msg = f"Invalid value for 'bounce-port-link': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_BOUNCE_PORT_LINK)}"
            error_msg += f"\n  → Example: bounce-port-link='{{ VALID_BODY_BOUNCE_PORT_LINK[0] }}'"
            return (False, error_msg)
    if "poe-reset" in payload:
        value = payload["poe-reset"]
        if value not in VALID_BODY_POE_RESET:
            desc = FIELD_DESCRIPTIONS.get("poe-reset", "")
            error_msg = f"Invalid value for 'poe-reset': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_POE_RESET)}"
            error_msg += f"\n  → Example: poe-reset='{{ VALID_BODY_POE_RESET[0] }}'"
            return (False, error_msg)
...
def validate_switch_controller_mac_policy_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_switch_controller_mac_policy_put(payload)
    """
    ...
    if "count" in payload:
        ...
        if value not in VALID_BODY_COUNT:
            return (
                False,
                f"Invalid value for 'count'='{value}'. Must be one of: {', '.join(VALID_BODY_COUNT)}",
            )
    if "bounce-port-link" in payload:
        value = payload["bounce-port-link"]
        if value not in VALID_BODY_BOUNCE_PORT_LINK:
            return (
                False,
                f"Invalid value for 'bounce-port-link'='{value}'. Must be one of: {', '.join(VALID_BODY_BOUNCE_PORT_LINK)}",
            )
    if "poe-reset" in payload:
        value = payload["poe-reset"]
        if value not in VALID_BODY_POE_RESET:
            return (
                False,
                f"Invalid value for 'poe-reset'='{value}'. Must be one of: {', '.join(VALID_BODY_POE_RESET)}",
            )
