...
def validate_ips_settings_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_ips_settings_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for ips/settings.
...
def validate_ips_settings_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["proxy-inline-ips"] = "invalid-value"
        >>> is_valid, error = validate_ips_settings_post(payload)
    ...
    if "proxy-inline-ips" in payload:
        value = payload["proxy-inline-ips"]
        if value not in VALID_BODY_PROXY_INLINE_IPS:
            desc = FIELD_DESCRIPTIONS.get("proxy-inline-ips", "")
            error_msg = f"Invalid value for 'proxy-inline-ips': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_PROXY_INLINE_IPS)}"
            error_msg += f"\n  → Example: proxy-inline-ips='{{ VALID_BODY_PROXY_INLINE_IPS[0] }}'"
            return (False, error_msg)
    if "ha-session-pickup" in payload:
        value = payload["ha-session-pickup"]
        if value not in VALID_BODY_HA_SESSION_PICKUP:
            desc = FIELD_DESCRIPTIONS.get("ha-session-pickup", "")
            error_msg = f"Invalid value for 'ha-session-pickup': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_HA_SESSION_PICKUP)}"
            error_msg += f"\n  → Example: ha-session-pickup='{{ VALID_BODY_HA_SESSION_PICKUP[0] }}'"
            return (False, error_msg)
...
def validate_ips_settings_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_ips_settings_put(payload)
    """
    ...
    if "proxy-inline-ips" in payload:
        ...
        if value not in VALID_BODY_PROXY_INLINE_IPS:
            return (
                False,
                f"Invalid value for 'proxy-inline-ips'='{value}'. Must be one of: {', '.join(VALID_BODY_PROXY_INLINE_IPS)}",
            )
    if "ha-session-pickup" in payload:
        value = payload["ha-session-pickup"]
        if value not in VALID_BODY_HA_SESSION_PICKUP:
            return (
                False,
                f"Invalid value for 'ha-session-pickup'='{value}'. Must be one of: {', '.join(VALID_BODY_HA_SESSION_PICKUP)}",
            )
