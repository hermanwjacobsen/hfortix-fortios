...
def validate_system_password_policy_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_system_password_policy_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for system/password_policy.
...
def validate_system_password_policy_post(
    ...
    **params: Any,
    ...
    if "apply-to" in payload:
        value = payload["apply-to"]
        if value not in VALID_BODY_APPLY_TO:
            desc = FIELD_DESCRIPTIONS.get("apply-to", "")
            error_msg = f"Invalid value for 'apply-to': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_APPLY_TO)}"
            error_msg += f"\n  → Example: apply-to='{{ VALID_BODY_APPLY_TO[0] }}'"
            return (False, error_msg)
    ...
    if "login-lockout-upon-weaker-encryption" in payload:
        value = payload["login-lockout-upon-weaker-encryption"]
        if value not in VALID_BODY_LOGIN_LOCKOUT_UPON_WEAKER_ENCRYPTION:
            desc = FIELD_DESCRIPTIONS.get("login-lockout-upon-weaker-encryption", "")
            error_msg = f"Invalid value for 'login-lockout-upon-weaker-encryption': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_LOGIN_LOCKOUT_UPON_WEAKER_ENCRYPTION)}"
            error_msg += f"\n  → Example: login-lockout-upon-weaker-encryption='{{ VALID_BODY_LOGIN_LOCKOUT_UPON_WEAKER_ENCRYPTION[0] }}'"
            return (False, error_msg)
...
def validate_system_password_policy_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_system_password_policy_put(payload)
    """
    ...
    if "apply-to" in payload:
        ...
        if value not in VALID_BODY_APPLY_TO:
            return (
                False,
                f"Invalid value for 'apply-to'='{value}'. Must be one of: {', '.join(VALID_BODY_APPLY_TO)}",
            )
    if "expire-status" in payload:
        value = payload["expire-status"]
        if value not in VALID_BODY_EXPIRE_STATUS:
    ...
    if "login-lockout-upon-weaker-encryption" in payload:
        ...
        if value not in VALID_BODY_LOGIN_LOCKOUT_UPON_WEAKER_ENCRYPTION:
            return (
                False,
                f"Invalid value for 'login-lockout-upon-weaker-encryption'='{value}'. Must be one of: {', '.join(VALID_BODY_LOGIN_LOCKOUT_UPON_WEAKER_ENCRYPTION)}",
            )
