...
def validate_system_zone_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_system_zone_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_system_zone_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for system/zone.
...
def validate_system_zone_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["intrazone"] = "invalid-value"
        >>> is_valid, error = validate_system_zone_post(payload)
    ...
    if "intrazone" in payload:
        value = payload["intrazone"]
        if value not in VALID_BODY_INTRAZONE:
            desc = FIELD_DESCRIPTIONS.get("intrazone", "")
            error_msg = f"Invalid value for 'intrazone': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_INTRAZONE)}"
            error_msg += f"\n  → Example: intrazone='{{ VALID_BODY_INTRAZONE[0] }}'"
...
def validate_system_zone_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_system_zone_put(payload)
    """
    ...
    if "intrazone" in payload:
        ...
        if value not in VALID_BODY_INTRAZONE:
            return (
                False,
                f"Invalid value for 'intrazone'='{value}'. Must be one of: {', '.join(VALID_BODY_INTRAZONE)}",
            )
