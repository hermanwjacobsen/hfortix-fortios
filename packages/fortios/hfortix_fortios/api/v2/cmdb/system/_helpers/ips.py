...
def validate_system_ips_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_system_ips_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for system/ips.
...
def validate_system_ips_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["override-signature-hold-by-id"] = "invalid-value"
        >>> is_valid, error = validate_system_ips_post(payload)
    ...
    if "override-signature-hold-by-id" in payload:
        value = payload["override-signature-hold-by-id"]
        if value not in VALID_BODY_OVERRIDE_SIGNATURE_HOLD_BY_ID:
            desc = FIELD_DESCRIPTIONS.get("override-signature-hold-by-id", "")
            error_msg = f"Invalid value for 'override-signature-hold-by-id': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_OVERRIDE_SIGNATURE_HOLD_BY_ID)}"
            error_msg += f"\n  → Example: override-signature-hold-by-id='{{ VALID_BODY_OVERRIDE_SIGNATURE_HOLD_BY_ID[0] }}'"
...
def validate_system_ips_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_system_ips_put(payload)
    """
    ...
    if "override-signature-hold-by-id" in payload:
        ...
        if value not in VALID_BODY_OVERRIDE_SIGNATURE_HOLD_BY_ID:
            return (
                False,
                f"Invalid value for 'override-signature-hold-by-id'='{value}'. Must be one of: {', '.join(VALID_BODY_OVERRIDE_SIGNATURE_HOLD_BY_ID)}",
            )
