...
VALID_BODY_PROXY_KEEP_ALIVE_MODE = [
    ...
    "re-authentication",  # Proxy keep-alive timeout begins when the user was authenticated.
]
...
VALID_BODY_LOG_SINGLE_CPU_HIGH = [
    "enable",  # Enable logging the event of a single CPU core reaching CPU usage threshold.
    "disable",  # Disable logging the event of a single CPU core reaching CPU usage threshold.
]
...
VALID_BODY_UPGRADE_REPORT = [
    "enable",  # Enable to automatically generate upgrade report when upgrading the firmware.
    "disable",  # Disable upgrade report generation when upgrading the firmware.
]
...
VALID_BODY_ADMIN_RESTRICT_LOCAL = [
    "all",  # Enable local admin authentication restriction including console.
    "non-console-only",  # Enable local admin authentication restriction excluding console.
...
VALID_BODY_ADMIN_SCP = [
    "enable",  # Enable SCP support for system configuration backup, restore, and firmware file upload.
    "disable",  # Disable SCP support for system configuration backup, restore, and firmware file upload.
]
...
VALID_BODY_FORTIEXTENDER_PROVISION_ON_AUTHORIZATION = [
    "enable",  # Enable FortiExtender firmware provision on authorization.
    "disable",  # Disable FortiExtender firmware provision on authorization.
]
...
VALID_BODY_SPECIAL_FILE_23_SUPPORT = [
    "disable",  # Disable detection of those special format files when using Data Loss Prevention.
    "enable",  # Enable detection of those special format files when using Data Loss Prevention.
]
...
VALID_BODY_REST_API_KEY_URL_QUERY = [
    "enable",  # Enable support for passing REST API keys through URL query parameters.
    "disable",  # Disable support for passing REST API keys through URL query parameters.
]
...
VALID_BODY_GEOIP_FULL_DB = [
    "enable",  # Full GeoDB will be downloaded. FortiView countries can be used.
    "disable",  # Full GeoDB will not be downloaded. FortiView countries cannot be used.
]
...
VALID_BODY_NPU_NEIGHBOR_UPDATE = [
    "enable",  # Enable sending of ARP/ICMP6 probing packets to update neighbors for offloaded sessions.
    "disable",  # Disable sending of ARP/ICMP6 probing packets to update neighbors for offloaded sessions.
]
...
VALID_BODY_INTERFACE_SUBNET_USAGE = [
    "disable",  # Disallow use of the interface-subnet setting in firewall addresses. Use in conjunction with the FortiGate REST API and when a large number of firewall addresses exist in the configuration.
    "enable",  # Allow use of the interface-subnet setting in firewall addresses.
]
VALID_BODY_FORTIGSLB_INTEGRATION = [
    "disable",  # Disable VIP and ZTNA server integration with the FortiGSLB cloud service.
    "enable",  # Enable VIP and ZTNA server integration with the FortiGSLB cloud service.
]
...
def validate_system_global__get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_system_global__get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for system/global_.
...
def validate_system_global__post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        ... }
        >>> is_valid, error = validate_system_global__post(payload)
        >>> assert is_valid == True
        
        >>> # ✅ Valid - With enum field
        >>> payload = {
        ...     "restart-time": True,
        ...     "language": "{'name': 'english', 'help': 'English.', 'label': 'English', 'description': 'English'}",  # Valid enum value
        ... }
        >>> is_valid, error = validate_system_global__post(payload)
        >>> assert is_valid == True
    ...
    if "geoip-full-db" in payload:
        value = payload["geoip-full-db"]
        if value not in VALID_BODY_GEOIP_FULL_DB:
            desc = FIELD_DESCRIPTIONS.get("geoip-full-db", "")
            error_msg = f"Invalid value for 'geoip-full-db': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_GEOIP_FULL_DB)}"
            error_msg += f"\n  → Example: geoip-full-db='{{ VALID_BODY_GEOIP_FULL_DB[0] }}'"
            return (False, error_msg)
...
def validate_system_global__put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_system_global__put(payload)
    """
    ...
    if "geoip-full-db" in payload:
        ...
        if value not in VALID_BODY_GEOIP_FULL_DB:
            return (
                False,
                f"Invalid value for 'geoip-full-db'='{value}'. Must be one of: {', '.join(VALID_BODY_GEOIP_FULL_DB)}",
            )
    if "early-tcp-npu-session" in payload:
        value = payload["early-tcp-npu-session"]
        if value not in VALID_BODY_EARLY_TCP_NPU_SESSION:
