...
def validate_system_auto_script_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_system_auto_script_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_system_auto_script_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for system/auto_script.
...
def validate_system_auto_script_post(
    ...
    **params: Any,
    ...
    if "start" in payload:
        value = payload["start"]
        if value not in VALID_BODY_START:
            desc = FIELD_DESCRIPTIONS.get("start", "")
            error_msg = f"Invalid value for 'start': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_START)}"
            error_msg += f"\n  → Example: start='{{ VALID_BODY_START[0] }}'"
...
def validate_system_auto_script_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_system_auto_script_put(payload)
    """
    ...
    if "start" in payload:
        ...
        if value not in VALID_BODY_START:
            return (
                False,
                f"Invalid value for 'start'='{value}'. Must be one of: {', '.join(VALID_BODY_START)}",
            )
