...
def validate_system_pppoe_interface_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_system_pppoe_interface_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_system_pppoe_interface_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for system/pppoe_interface.
...
def validate_system_pppoe_interface_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload = {
        ...     "device": True,  # Name for the physical interface.
        ... }
        ...
        >>> payload["dial-on-demand"] = "invalid-value"
        >>> is_valid, error = validate_system_pppoe_interface_post(payload)
    ...
    if "dial-on-demand" in payload:
        value = payload["dial-on-demand"]
        if value not in VALID_BODY_DIAL_ON_DEMAND:
            desc = FIELD_DESCRIPTIONS.get("dial-on-demand", "")
            error_msg = f"Invalid value for 'dial-on-demand': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_DIAL_ON_DEMAND)}"
            error_msg += f"\n  → Example: dial-on-demand='{{ VALID_BODY_DIAL_ON_DEMAND[0] }}'"
...
def validate_system_pppoe_interface_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_system_pppoe_interface_put(payload)
    """
    ...
    if "dial-on-demand" in payload:
        ...
        if value not in VALID_BODY_DIAL_ON_DEMAND:
            return (
                False,
                f"Invalid value for 'dial-on-demand'='{value}'. Must be one of: {', '.join(VALID_BODY_DIAL_ON_DEMAND)}",
            )
    if "ipv6" in payload:
        value = payload["ipv6"]
        if value not in VALID_BODY_IPV6:
