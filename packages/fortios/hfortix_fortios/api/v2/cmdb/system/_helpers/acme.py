...
def validate_system_acme_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_system_acme_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for system/acme.
...
def validate_system_acme_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["use-ha-direct"] = "invalid-value"
        >>> is_valid, error = validate_system_acme_post(payload)
    ...
    if "use-ha-direct" in payload:
        value = payload["use-ha-direct"]
        if value not in VALID_BODY_USE_HA_DIRECT:
            desc = FIELD_DESCRIPTIONS.get("use-ha-direct", "")
            error_msg = f"Invalid value for 'use-ha-direct': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_USE_HA_DIRECT)}"
            error_msg += f"\n  → Example: use-ha-direct='{{ VALID_BODY_USE_HA_DIRECT[0] }}'"
...
def validate_system_acme_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_system_acme_put(payload)
    """
    ...
    if "use-ha-direct" in payload:
        ...
        if value not in VALID_BODY_USE_HA_DIRECT:
            return (
                False,
                f"Invalid value for 'use-ha-direct'='{value}'. Must be one of: {', '.join(VALID_BODY_USE_HA_DIRECT)}",
            )
