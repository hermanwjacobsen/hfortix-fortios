...
def validate_videofilter_profile_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_videofilter_profile_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_videofilter_profile_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for videofilter/profile.
...
def validate_videofilter_profile_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload = {
        ...     "name": True,  # Name.
        ...     "filters": True,  # YouTube filter entries.
        ... }
    ...
    if "youtube" in payload:
        ...
        if value not in VALID_BODY_YOUTUBE:
            desc = FIELD_DESCRIPTIONS.get("youtube", "")
            error_msg = f"Invalid value for 'youtube': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_YOUTUBE)}"
            error_msg += f"\n  → Example: youtube='{{ VALID_BODY_YOUTUBE[0] }}'"
            return (False, error_msg)
    if "vimeo" in payload:
        value = payload["vimeo"]
        if value not in VALID_BODY_VIMEO:
            desc = FIELD_DESCRIPTIONS.get("vimeo", "")
            error_msg = f"Invalid value for 'vimeo': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_VIMEO)}"
            error_msg += f"\n  → Example: vimeo='{{ VALID_BODY_VIMEO[0] }}'"
            return (False, error_msg)
    if "dailymotion" in payload:
        value = payload["dailymotion"]
        if value not in VALID_BODY_DAILYMOTION:
            desc = FIELD_DESCRIPTIONS.get("dailymotion", "")
            error_msg = f"Invalid value for 'dailymotion': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_DAILYMOTION)}"
            error_msg += f"\n  → Example: dailymotion='{{ VALID_BODY_DAILYMOTION[0] }}'"
            return (False, error_msg)
...
def validate_videofilter_profile_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_videofilter_profile_put(payload)
    """
    ...
    if "youtube" in payload:
        ...
        if value not in VALID_BODY_YOUTUBE:
            return (
                False,
                f"Invalid value for 'youtube'='{value}'. Must be one of: {', '.join(VALID_BODY_YOUTUBE)}",
            )
    if "vimeo" in payload:
        value = payload["vimeo"]
        if value not in VALID_BODY_VIMEO:
            return (
                False,
                f"Invalid value for 'vimeo'='{value}'. Must be one of: {', '.join(VALID_BODY_VIMEO)}",
            )
    if "dailymotion" in payload:
        value = payload["dailymotion"]
        if value not in VALID_BODY_DAILYMOTION:
            return (
                False,
                f"Invalid value for 'dailymotion'='{value}'. Must be one of: {', '.join(VALID_BODY_DAILYMOTION)}",
            )
