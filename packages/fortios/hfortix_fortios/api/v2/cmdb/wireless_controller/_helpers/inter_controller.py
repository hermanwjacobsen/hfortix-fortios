"""
Validation helpers for wireless_controller/inter_controller endpoint.
...
def validate_wireless_controller_inter_controller_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_wireless_controller_inter_controller_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for wireless_controller/inter_controller.
...
def validate_wireless_controller_inter_controller_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["inter-controller-mode"] = "invalid-value"
        >>> is_valid, error = validate_wireless_controller_inter_controller_post(payload)
    ...
    if "inter-controller-mode" in payload:
        value = payload["inter-controller-mode"]
        if value not in VALID_BODY_INTER_CONTROLLER_MODE:
            desc = FIELD_DESCRIPTIONS.get("inter-controller-mode", "")
            error_msg = f"Invalid value for 'inter-controller-mode': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_INTER_CONTROLLER_MODE)}"
            error_msg += f"\n  → Example: inter-controller-mode='{{ VALID_BODY_INTER_CONTROLLER_MODE[0] }}'"
    ...
    if "inter-controller-pri" in payload:
        value = payload["inter-controller-pri"]
        if value not in VALID_BODY_INTER_CONTROLLER_PRI:
            desc = FIELD_DESCRIPTIONS.get("inter-controller-pri", "")
            error_msg = f"Invalid value for 'inter-controller-pri': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_INTER_CONTROLLER_PRI)}"
            error_msg += f"\n  → Example: inter-controller-pri='{{ VALID_BODY_INTER_CONTROLLER_PRI[0] }}'"
            return (False, error_msg)
...
def validate_wireless_controller_inter_controller_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_wireless_controller_inter_controller_put(payload)
    """
    ...
    if "inter-controller-mode" in payload:
        ...
        if value not in VALID_BODY_INTER_CONTROLLER_MODE:
            return (
                False,
                f"Invalid value for 'inter-controller-mode'='{value}'. Must be one of: {', '.join(VALID_BODY_INTER_CONTROLLER_MODE)}",
            )
    if "l3-roaming" in payload:
        value = payload["l3-roaming"]
        if value not in VALID_BODY_L3_ROAMING:
    ...
    if "inter-controller-pri" in payload:
        ...
        if value not in VALID_BODY_INTER_CONTROLLER_PRI:
            return (
                False,
                f"Invalid value for 'inter-controller-pri'='{value}'. Must be one of: {', '.join(VALID_BODY_INTER_CONTROLLER_PRI)}",
            )
