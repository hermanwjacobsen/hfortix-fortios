"""
Validation helpers for wireless_controller/wag_profile endpoint.
...
def validate_wireless_controller_wag_profile_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_wireless_controller_wag_profile_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_wireless_controller_wag_profile_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for wireless_controller/wag_profile.
...
def validate_wireless_controller_wag_profile_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["tunnel-type"] = "invalid-value"
        >>> is_valid, error = validate_wireless_controller_wag_profile_post(payload)
    ...
    if "tunnel-type" in payload:
        value = payload["tunnel-type"]
        if value not in VALID_BODY_TUNNEL_TYPE:
            desc = FIELD_DESCRIPTIONS.get("tunnel-type", "")
            error_msg = f"Invalid value for 'tunnel-type': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_TUNNEL_TYPE)}"
            error_msg += f"\n  → Example: tunnel-type='{{ VALID_BODY_TUNNEL_TYPE[0] }}'"
...
def validate_wireless_controller_wag_profile_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_wireless_controller_wag_profile_put(payload)
    """
    ...
    if "tunnel-type" in payload:
        ...
        if value not in VALID_BODY_TUNNEL_TYPE:
            return (
                False,
                f"Invalid value for 'tunnel-type'='{value}'. Must be one of: {', '.join(VALID_BODY_TUNNEL_TYPE)}",
            )
