"""
Validation helpers for wireless_controller/syslog_profile endpoint.
...
def validate_wireless_controller_syslog_profile_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_wireless_controller_syslog_profile_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_wireless_controller_syslog_profile_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for wireless_controller/syslog_profile.
...
def validate_wireless_controller_syslog_profile_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["server-status"] = "invalid-value"
        >>> is_valid, error = validate_wireless_controller_syslog_profile_post(payload)
    ...
    if "server-status" in payload:
        ...
        if value not in VALID_BODY_SERVER_STATUS:
            desc = FIELD_DESCRIPTIONS.get("server-status", "")
            error_msg = f"Invalid value for 'server-status': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
    ...
    if "log-level" in payload:
        value = payload["log-level"]
        if value not in VALID_BODY_LOG_LEVEL:
            desc = FIELD_DESCRIPTIONS.get("log-level", "")
            error_msg = f"Invalid value for 'log-level': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_LOG_LEVEL)}"
            error_msg += f"\n  → Example: log-level='{{ VALID_BODY_LOG_LEVEL[0] }}'"
            return (False, error_msg)
...
def validate_wireless_controller_syslog_profile_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_wireless_controller_syslog_profile_put(payload)
    """
    ...
    if "log-level" in payload:
        ...
        if value not in VALID_BODY_LOG_LEVEL:
            return (
                False,
                f"Invalid value for 'log-level'='{value}'. Must be one of: {', '.join(VALID_BODY_LOG_LEVEL)}",
            )
