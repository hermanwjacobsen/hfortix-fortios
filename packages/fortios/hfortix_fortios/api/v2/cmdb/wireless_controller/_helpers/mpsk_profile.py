"""
Validation helpers for wireless_controller/mpsk_profile endpoint.
...
def validate_wireless_controller_mpsk_profile_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_wireless_controller_mpsk_profile_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_wireless_controller_mpsk_profile_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for wireless_controller/mpsk_profile.
...
def validate_wireless_controller_mpsk_profile_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["mpsk-external-server-auth"] = "invalid-value"
        >>> is_valid, error = validate_wireless_controller_mpsk_profile_post(payload)
    ...
    if "mpsk-external-server-auth" in payload:
        value = payload["mpsk-external-server-auth"]
        if value not in VALID_BODY_MPSK_EXTERNAL_SERVER_AUTH:
            desc = FIELD_DESCRIPTIONS.get("mpsk-external-server-auth", "")
            error_msg = f"Invalid value for 'mpsk-external-server-auth': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_MPSK_EXTERNAL_SERVER_AUTH)}"
            error_msg += f"\n  → Example: mpsk-external-server-auth='{{ VALID_BODY_MPSK_EXTERNAL_SERVER_AUTH[0] }}'"
            return (False, error_msg)
    if "mpsk-type" in payload:
        value = payload["mpsk-type"]
        if value not in VALID_BODY_MPSK_TYPE:
            desc = FIELD_DESCRIPTIONS.get("mpsk-type", "")
            error_msg = f"Invalid value for 'mpsk-type': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_MPSK_TYPE)}"
            error_msg += f"\n  → Example: mpsk-type='{{ VALID_BODY_MPSK_TYPE[0] }}'"
            return (False, error_msg)
...
def validate_wireless_controller_mpsk_profile_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_wireless_controller_mpsk_profile_put(payload)
    """
    ...
    if "mpsk-external-server-auth" in payload:
        ...
        if value not in VALID_BODY_MPSK_EXTERNAL_SERVER_AUTH:
            return (
                False,
                f"Invalid value for 'mpsk-external-server-auth'='{value}'. Must be one of: {', '.join(VALID_BODY_MPSK_EXTERNAL_SERVER_AUTH)}",
            )
    if "mpsk-type" in payload:
        value = payload["mpsk-type"]
        if value not in VALID_BODY_MPSK_TYPE:
            return (
                False,
                f"Invalid value for 'mpsk-type'='{value}'. Must be one of: {', '.join(VALID_BODY_MPSK_TYPE)}",
            )
