"""
Validation helpers for wireless_controller/apcfg_profile endpoint.
...
def validate_wireless_controller_apcfg_profile_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_wireless_controller_apcfg_profile_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_wireless_controller_apcfg_profile_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for wireless_controller/apcfg_profile.
...
def validate_wireless_controller_apcfg_profile_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["ap-family"] = "invalid-value"
        >>> is_valid, error = validate_wireless_controller_apcfg_profile_post(payload)
    ...
    if "ap-family" in payload:
        value = payload["ap-family"]
        if value not in VALID_BODY_AP_FAMILY:
            desc = FIELD_DESCRIPTIONS.get("ap-family", "")
            error_msg = f"Invalid value for 'ap-family': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AP_FAMILY)}"
            error_msg += f"\n  → Example: ap-family='{{ VALID_BODY_AP_FAMILY[0] }}'"
            return (False, error_msg)
    if "ac-type" in payload:
        value = payload["ac-type"]
        if value not in VALID_BODY_AC_TYPE:
            desc = FIELD_DESCRIPTIONS.get("ac-type", "")
            error_msg = f"Invalid value for 'ac-type': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_AC_TYPE)}"
            error_msg += f"\n  → Example: ac-type='{{ VALID_BODY_AC_TYPE[0] }}'"
            return (False, error_msg)
...
def validate_wireless_controller_apcfg_profile_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_wireless_controller_apcfg_profile_put(payload)
    """
    ...
    if "ap-family" in payload:
        ...
        if value not in VALID_BODY_AP_FAMILY:
            return (
                False,
                f"Invalid value for 'ap-family'='{value}'. Must be one of: {', '.join(VALID_BODY_AP_FAMILY)}",
            )
    if "ac-type" in payload:
        value = payload["ac-type"]
        if value not in VALID_BODY_AC_TYPE:
            return (
                False,
                f"Invalid value for 'ac-type'='{value}'. Must be one of: {', '.join(VALID_BODY_AC_TYPE)}",
            )
