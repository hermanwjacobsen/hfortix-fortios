...
def validate_application_group_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_application_group_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_application_group_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for application/group.
...
def validate_application_group_post(
    ...
    **params: Any,
    ...
    if "popularity" in payload:
        value = payload["popularity"]
        if value not in VALID_BODY_POPULARITY:
            desc = FIELD_DESCRIPTIONS.get("popularity", "")
            error_msg = f"Invalid value for 'popularity': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_POPULARITY)}"
            error_msg += f"\n  → Example: popularity='{{ VALID_BODY_POPULARITY[0] }}'"
            return (False, error_msg)
...
def validate_application_group_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_application_group_put(payload)
    """
    ...
    if "popularity" in payload:
        ...
        if value not in VALID_BODY_POPULARITY:
            return (
                False,
                f"Invalid value for 'popularity'='{value}'. Must be one of: {', '.join(VALID_BODY_POPULARITY)}",
            )
