...
def validate_firewall_auth_portal_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_firewall_auth_portal_get()
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for firewall/auth_portal.
...
def validate_firewall_auth_portal_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload["proxy-auth"] = "invalid-value"
        >>> is_valid, error = validate_firewall_auth_portal_post(payload)
    ...
    if "proxy-auth" in payload:
        value = payload["proxy-auth"]
        if value not in VALID_BODY_PROXY_AUTH:
            desc = FIELD_DESCRIPTIONS.get("proxy-auth", "")
            error_msg = f"Invalid value for 'proxy-auth': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_PROXY_AUTH)}"
            error_msg += f"\n  → Example: proxy-auth='{{ VALID_BODY_PROXY_AUTH[0] }}'"
...
def validate_firewall_auth_portal_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_firewall_auth_portal_put(payload)
    """
    ...
    if "proxy-auth" in payload:
        ...
        if value not in VALID_BODY_PROXY_AUTH:
            return (
                False,
                f"Invalid value for 'proxy-auth'='{value}'. Must be one of: {', '.join(VALID_BODY_PROXY_AUTH)}",
            )
