...
def validate_firewall_on_demand_sniffer_get(
    ...
    **params: Any,
    ...
    Examples:
        >>> # Valid - Get all items
        >>> is_valid, error = validate_firewall_on_demand_sniffer_get()
        >>> assert is_valid == True
        
        >>> # Valid - Get specific item by name
        >>> is_valid, error = validate_firewall_on_demand_sniffer_get(name="test-item")
        >>> assert is_valid == True
...
def validate_required_fields(payload: dict) -> tuple[bool, str | None]:
    """
    Validate required fields for firewall/on_demand_sniffer.
...
def validate_firewall_on_demand_sniffer_post(
    ...
    **params: Any,
    ...
    Examples:
        ...
        >>> payload = {
        ...     "interface": True,  # Interface name that on-demand packet sniffer will 
        ... }
        ...
        >>> payload["non-ip-packet"] = "invalid-value"
        >>> is_valid, error = validate_firewall_on_demand_sniffer_post(payload)
    ...
    if "non-ip-packet" in payload:
        value = payload["non-ip-packet"]
        if value not in VALID_BODY_NON_IP_PACKET:
            desc = FIELD_DESCRIPTIONS.get("non-ip-packet", "")
            error_msg = f"Invalid value for 'non-ip-packet': '{value}'"
            if desc:
                error_msg += f"\n  → Description: {desc}"
            error_msg += f"\n  → Valid options: {', '.join(repr(v) for v in VALID_BODY_NON_IP_PACKET)}"
            error_msg += f"\n  → Example: non-ip-packet='{{ VALID_BODY_NON_IP_PACKET[0] }}'"
...
def validate_firewall_on_demand_sniffer_put(
    ...
    **params: Any,
    ...
    Example:
        ...
        >>> is_valid, error = validate_firewall_on_demand_sniffer_put(payload)
    """
    ...
    if "non-ip-packet" in payload:
        ...
        if value not in VALID_BODY_NON_IP_PACKET:
            return (
                False,
                f"Invalid value for 'non-ip-packet'='{value}'. Must be one of: {', '.join(VALID_BODY_NON_IP_PACKET)}",
            )
