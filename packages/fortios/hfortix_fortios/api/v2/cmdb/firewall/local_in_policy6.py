...
class LocalInPolicy6:
    ...
    def get(
        ...
        **kwargs: Any,
        ...
        Examples:
            ...
            >>> schema = fgt.api.cmdb.firewall_local_in_policy6.get(action="schema")
    ...
    def help(field_name: str | None = None) -> str:
        ...
        Examples:
            ...
            >>> print(LocalInPolicy6.help())
            
            >>> # Get field information
            >>> print(LocalInPolicy6.help("policyid"))
        """
        from ._helpers.local_in_policy6 import (
            get_schema_info,
            get_field_metadata,
        )
    ...
    def fields(detailed: bool = False) -> Union[list[str], dict[str, dict]]:
        ...
        Examples:
            >>> # Simple list
            >>> fields = LocalInPolicy6.fields()
            >>> print(f"Available fields: {len(fields)}")
            
            >>> # Detailed info
            >>> fields = LocalInPolicy6.fields(detailed=True)
            >>> for name, meta in fields.items():
            ...     print(f"{name}: {meta['type']}")
        """
        from ._helpers.local_in_policy6 import get_all_fields, get_field_metadata
    ...
    def field_info(field_name: str) -> dict[str, Any] | None:
        ...
        Examples:
            >>> info = LocalInPolicy6.field_info("policyid")
            >>> print(f"Type: {info['type']}")
            >>> if 'options' in info:
            ...     print(f"Options: {info['options']}")
        """
        from ._helpers.local_in_policy6 import get_field_metadata
    ...
    def validate_field(field_name: str, value: Any) -> tuple[bool, str | None]:
        ...
        Examples:
            >>> is_valid, error = LocalInPolicy6.validate_field("policyid", "test")
            >>> if not is_valid:
            ...     print(f"Validation error: {error}")
        """
        from ._helpers.local_in_policy6 import validate_field_value
    ...
    def required_fields() -> list[str]:
        ...
        Examples:
            >>> required = LocalInPolicy6.required_fields()
            >>> print(f"Required fields: {', '.join(required)}")
        """
        from ._helpers.local_in_policy6 import REQUIRED_FIELDS
    ...
    def defaults() -> dict[str, Any]:
        ...
        Examples:
            >>> defaults = LocalInPolicy6.defaults()
            >>> print(f"Fields with defaults: {len(defaults)}")
            >>> # Use as starting point for payload
            >>> payload = defaults.copy()
            >>> payload['name'] = 'my-custom-name'
        """
        from ._helpers.local_in_policy6 import FIELDS_WITH_DEFAULTS
    ...
    def schema() -> dict[str, Any]:
        ...
        Examples:
            >>> schema = LocalInPolicy6.schema()
            >>> print(f"Endpoint: {schema['endpoint']}")
            >>> print(f"Total fields: {schema['total_fields']}")
            >>> print(f"Primary key: {schema.get('mkey', 'N/A')}")
        """
        from ._helpers.local_in_policy6 import get_schema_info
