...
FIELD_TYPES = {
    ...
    "default-gw": "ipv4-address",  # IPv4 address of default route gateway to use for traffic exiting the interface.
    "default-gw-priority": "integer",  # Priority for default gateway route. A higher priority number signifies a less pr
    ...
    "monitor-min": "integer",  # Minimum number of links to become degraded before activating this interface.  Ze
    "monitor-hold-down-type": "option",  # Recovery time method when primary interface re-establishes.
    "monitor-hold-down-delay": "integer",  # Time to wait in seconds before recovery once primary re-establishes.
    "monitor-hold-down-weekday": "option",  # Day of the week to recover once primary re-establishes.
    "monitor-hold-down-time": "user",  # Time of day at which to fail back to primary after it re-establishes.
    ...
    "packet-redistribution": "option",  # Enable/disable packet distribution (RPS) on the IPsec interface.
    "peer-egress-shaping": "option",  # Enable/disable peer egress shaping.
    "peer-egress-shaping-value": "integer",  # Configure outbound bandwidth to use for peer egress shaping in kbps (0 - 8000000
    ...
    "ip-fragmentation": "option",  # Determine whether IP packets are fragmented before or after IPsec encapsulation.
    ...
    "auto-discovery-psk": "option",  # Enable/disable use of pre-shared secrets for authentication of auto-discovery tu
    "auto-discovery-shortcuts": "option",  # Control deletion of child short-cut tunnels when the parent tunnel goes down.
    "auto-discovery-crossover": "option",  # Allow/block set-up of short-cut tunnels between different network IDs.
    "auto-discovery-offer-interval": "integer",  # Interval between shortcut offer messages in seconds (1 - 300, default = 5).
    "auto-discovery-dialup-placeholder": "option",  # Control if this dynamic gateway is used for shortcut connections only.
