"""
CMDB Endpoint Tests: log.null-device/filter
Tests null-device filter configuration (VDOM scope).
...
def test_get():
    """Get and store original null-device filter config"""
    global original_config
    
    print("\n1. Getting original null-device filter config...")
    result = fgt.api.cmdb.log.null_device.filter.get(vdom="test")
    ...
    print(f"✓ Original severity: {original_config['severity']}, anomaly: {original_config['anomaly']}")
...
def test_put_change_severity():
    ...
    result = fgt.api.cmdb.log.null_device.filter.put(
    ...
    # Verify change
    verify = fgt.api.cmdb.log.null_device.filter.get(vdom="test")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "="*50)
        print("All null-device filter tests passed!")
        print("="*50)
