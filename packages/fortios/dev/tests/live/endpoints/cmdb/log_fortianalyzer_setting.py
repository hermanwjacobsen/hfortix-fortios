"""
CMDB Endpoint Tests: log.fortianalyzer/setting
Tests FortiAnalyzer global settings configuration.
...
def test_get():
    """Get and store original FortiAnalyzer setting config"""
    global original_config
    
    print("\n1. Getting original FortiAnalyzer setting config...")
    result = fgt.api.cmdb.log.fortianalyzer.setting.get()
...
def test_put_disable_ips_archive():
    ...
    result = fgt.api.cmdb.log.fortianalyzer.setting.put(
    ...
    # Verify change
    verify = fgt.api.cmdb.log.fortianalyzer.setting.get()
    assert verify is not None, "Failed to get updated config"
    assert verify.ips_archive == "disable", f"IPS archive not updated, got: {verify.ips_archive}"
...
def test_put_enable_cert_verification():
    ...
    verify = fgt.api.cmdb.log.fortianalyzer.setting.get()
    assert verify is not None, "Failed to get updated config"
    assert verify.certificate_verification == "enable", f"Certificate verification not updated, got: {verify.certificate_verification}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "="*50)
        print("All FortiAnalyzer setting tests passed!")
        print("="*50)
