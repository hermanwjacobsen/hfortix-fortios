"""
CMDB Endpoint Tests: log.memory/filter
Tests memory buffer filter configuration.
...
def test_get():
    """Get and store original memory filter config"""
    global original_config
    
    print("\n1. Getting original memory filter config...")
    result = fgt.api.cmdb.log.memory.filter.get(vdom="test")
...
def test_put_change_severity():
    ...
    result = fgt.api.cmdb.log.memory.filter.put(
        vdom="test",
    ...
    # Verify change
    verify = fgt.api.cmdb.log.memory.filter.get(vdom="test")
