"""
CMDB Endpoint Tests: log/threat-weight
Tests threat weight settings configuration (VDOM scope).
...
def test_get():
    """Get and store original threat-weight config"""
    global original_config
    
    print("\n1. Getting original threat-weight config...")
    result = fgt.api.cmdb.log.threat_weight.get(vdom="test")
    
    assert result is not None, "Failed to get threat-weight config"
    assert result.http_status == "success", "Failed to get threat-weight config"
...
def test_put_change_status():
    ...
    # Verify change
    verify = fgt.api.cmdb.log.threat_weight.get(vdom="test")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "="*50)
        print("All threat-weight tests passed!")
        print("="*50)
