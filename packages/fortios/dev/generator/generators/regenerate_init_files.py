...
def _get_extras_registry(api_base: Path) -> dict[str, dict[str, object]]:
    """
    Load the EXTRA_METHODS registry from _extras.py if it exists.
    
    Args:
        api_base: Path to the api/v2 directory
        
    Returns:
        Dictionary mapping endpoint paths to their extra methods, or empty dict
    """
    extras_file = api_base / "_extras.py"
    if not extras_file.exists():
        return {}
    
    # Parse the file to extract EXTRA_METHODS keys without importing
    # This avoids import issues during generation
    content = extras_file.read_text()
    
    # Look for EXTRA_METHODS dict definition and extract the keys
    import ast
    
    try:
        # Find the EXTRA_METHODS assignment (could be Assign or AnnAssign for type-annotated)
        tree = ast.parse(content)
        for node in ast.walk(tree):
            # Check for regular assignment: EXTRA_METHODS = {...}
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == "EXTRA_METHODS":
                        if isinstance(node.value, ast.Dict):
                            ...
                            for key in node.value.keys:
                                if isinstance(key, ast.Constant) and isinstance(key.value, str):
                                    result[key.value] = {}
                            return result
            # Check for annotated assignment: EXTRA_METHODS: dict[...] = {...}
            elif isinstance(node, ast.AnnAssign):
                if isinstance(node.target, ast.Name) and node.target.id == "EXTRA_METHODS":
...
def _generate_subdir_wrapper_init(
    subdir: Path, 
    base_module_name: str, 
    class_name: str, 
    endpoint_path: str = "",
    extras_registry: dict[str, dict] | None = None,
):
    """
    ...
    Args:
        ...
        endpoint_path: Dot-separated endpoint path (e.g., 'monitor.system.config_revision')
        extras_registry: Dictionary of endpoint paths that have extras defined
    ...
    # Default extras_registry if not provided
    if extras_registry is None:
        extras_registry = {}
    
    # Check if this endpoint has extras defined
    has_extras = endpoint_path and endpoint_path in extras_registry
    
    # Calculate relative import depth for _extras.py
    # From e.g., monitor/system/config_revision/__init__.py to api/v2/_extras.py
    # subdir is like: .../api/v2/monitor/system/config_revision/
    # Need to go up to api/v2/ which is 3 levels for this example
    relative_depth = len(subdir.relative_to(subdir.parents[2]).parts) + 1  # +1 to get to v2 level
    extras_import_prefix = "." * relative_depth
    ...
    # Import base class from *_base.py file in parent directory
    # base_module_name already includes '_base' suffix (e.g., 'application_list_base')
    init_content += f"from ..{base_module_name} import {class_name} as {class_name}Base\n"
    
    # Import action classes
    init_content += "\n".join(action_imports) + "\n"
    
    # Add extras import if needed
    if has_extras:
        init_content += f"from {extras_import_prefix}_extras import apply_extra_methods\n"
    ...
    # Apply extras if defined
    if has_extras:
        init_content += f'\n\n# Apply extra methods from _extras.py\napply_extra_methods("{endpoint_path}", {class_name})\n'
    
    # Write file
    init_file = subdir / "__init__.py"
    init_file.write_text(init_content)
    
    # Generate stub file (__init__.pyi) for Pylance/IDE support
    stub_content = f'"""FortiOS CMDB - {class_name} category (stub)"""\n\n'
    stub_content += "from typing import Any\n"
    stub_content += f"from ..{base_module_name} import {class_name} as {class_name}Base\n"
    
    # Import action classes for stubs
    for action_module, action_class in sorted(action_attrs):
        stub_content += f"from .{action_module} import {action_class}\n"
    
    stub_content += "\n"
    stub_content += f"class {class_name}({class_name}Base):\n"
    stub_content += f'    """{class_name} endpoints wrapper for CMDB API."""\n\n'
    
    # Add action attributes with type hints
    for action_module, action_class in sorted(action_attrs):
        stub_content += f"    {action_module}: {action_class}\n"
    
    stub_content += "\n"
    stub_content += "    def __init__(self, client: Any) -> None:\n"
    stub_content += '        """Initialize endpoint with HTTP client."""\n'
    stub_content += "        ...\n"
    
    # Add diff method stub if extras are defined for this endpoint
    if has_extras and endpoint_path == "monitor.system.config_revision":
        stub_content += "\n"
        stub_content += "    def diff(\n"
        stub_content += "        self,\n"
        stub_content += "        config_id1: int,\n"
        stub_content += "        config_id2: int,\n"
        stub_content += "        context_lines: int = ...,\n"
        stub_content += "        as_html: bool = ...,\n"
        stub_content += "        show_all: bool = ...,\n"
        stub_content += "    ) -> str:\n"
        stub_content += '        """\n'
        stub_content += "        Compare two configuration revisions and return the diff.\n"
        stub_content += "        \n"
        stub_content += "        By default, filters out sensitive fields (passwords, keys, etc.).\n"
        stub_content += "        Use show_all=True to include all fields.\n"
        stub_content += '        """\n'
        stub_content += "        ...\n"
    
    stub_file = subdir / "__init__.pyi"
    stub_file.write_text(stub_content)
    
    extras_note = " (+extras)" if has_extras else ""
    print(f"      ✨ Generated wrapper {subdir.name}/__init__.py + .pyi (inherits from {base_module_name}, {len(action_attrs)} actions{extras_note})")


def regenerate_category_init(
    category_dir: Path, 
    base_dir: Path, 
    skip_wrapper_subdirs: set | None = None,
    ...
    Args:
        ...
        skip_wrapper_subdirs: Set of subdirectory names that have already been processed as wrappers
        api_base: Path to the api/v2 directory (for loading extras)
        extras_registry: Pre-loaded extras registry (optional, will be loaded from api_base if not provided)
    """
    if skip_wrapper_subdirs is None:
        skip_wrapper_subdirs = set()
    
    # Load extras registry if not provided
    if extras_registry is None and api_base is not None:
        extras_registry = _get_extras_registry(api_base)
    elif extras_registry is None:
        extras_registry = {}
    ...
    # Track subdirectory names
    subdir_names = {d.name for d in subdirectories}
    
    # Track _base files that have corresponding subdirectories
    # These get a wrapper __init__.py that inherits from the base class
    # Pattern: endpoint_base.py + endpoint/__init__.py → class Endpoint(EndpointBase)
    ...
    for ep_file in endpoint_files:
        if ep_file.stem.endswith('_base'):
            ...
            if base_name in subdir_names:
                base_files_with_subdirs[base_name] = ep_file.stem
                print(f"    🔍 Detected _base pattern: {ep_file.stem} + {base_name}/ directory")
    
    if base_files_with_subdirs:
        print(f"    📋 Found {len(base_files_with_subdirs)} _base files with matching subdirs: {list(base_files_with_subdirs.keys())}")
    ...
    for subdir in sorted(subdirectories):
        ...
        # Generate wrapper __init__.py if this subdirectory has a corresponding _base file
        if subdir_name in base_files_with_subdirs:
            # Compute endpoint path for extras lookup
            # e.g., for /api/v2/monitor/system/config_revision -> "monitor.system.config_revision"
            endpoint_path = ""
            if api_base is not None:
                try:
                    relative_path = subdir.relative_to(api_base)
                    # Convert path parts to dot-separated string with underscores
                    endpoint_path = ".".join(p.replace("-", "_") for p in relative_path.parts)
                except ValueError:
                    pass
            
            print(f"    🔧 Generating wrapper {subdir_name}/__init__.py inheriting from {base_files_with_subdirs[subdir_name]}")
            _generate_subdir_wrapper_init(
                subdir,
                base_files_with_subdirs[subdir_name],
                subdir_class_name,
                endpoint_path=endpoint_path,
                extras_registry=extras_registry,
            )
    ...
    # Return the set of subdirectories that have wrappers (those with _base files)
    return set(base_files_with_subdirs.keys())


if __name__ == "__main__":
    # Base directory - resolve from script location
    script_path = Path(__file__).resolve()
    # From .dev/generator/generators/regenerate_init_files.py:
    # parent = .dev/generator/generators/
    # parent.parent = .dev/generator/
    # parent.parent.parent = .dev/
    # parent.parent.parent.parent = fortinet root
    ...
    # Load extras registry once for all
    extras_registry = _get_extras_registry(api_base)
    if extras_registry:
        print(f"📦 Found extras for: {list(extras_registry.keys())}\n")
    ...
    for api_type in api_types:
        ...
        regenerate_category_init(base_dir, api_base, api_base=api_base, extras_registry=extras_registry)
        
        for category_dir in sorted(category_dirs):
            wrapper_subdirs = regenerate_category_init(
                category_dir, base_dir, api_base=api_base, extras_registry=extras_registry
            ) or set()
            
            # Also process subdirectories recursively, but COMPLETELY SKIP those that have wrappers
            def process_subdirectories(parent_dir: Path, base: Path, skip_set: set):
                ...
                for subdir in subdirs:
                    ...
                    wrapper_subdirs_child = regenerate_category_init(
                        subdir, base, api_base=api_base, extras_registry=extras_registry
                    ) or set()
            ...
            process_subdirectories(category_dir, base_dir, wrapper_subdirs)
