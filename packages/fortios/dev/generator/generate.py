...
class CodeGenerator:
    ...
    def __init__(
        ...
        self.log_gen = LogEndpointGenerator(
            schema_dir=self.schema_dir,  # Use the version-specific schema directory
    ...
    def _regenerate_category_inits(self):
        ...
        def process_subdirectories(parent_dir: Path, base: Path, skip_set: set, api_base: Path):
            ...
            for subdir in subdirs:
                ...
                wrapper_subdirs_child = regenerate_category_init(subdir, base, api_base=api_base) or set()
                
                # Merge child wrapper subdirs with parent skip set
                combined_skip_set = skip_set | wrapper_subdirs_child
                process_subdirectories(subdir, base, combined_skip_set, api_base)  # Recurse with combined skip set
        ...
        for cat in categories:
            ...
            regenerate_category_init(category_dir, self.output_dir, api_base=self.output_dir)
            
            for subdir in sorted(category_subdirs):
                wrapper_subdirs = regenerate_category_init(subdir, category_dir, api_base=self.output_dir) or set()
                
                # Process subdirectories recursively, skipping wrappers
                process_subdirectories(subdir, category_dir, wrapper_subdirs, self.output_dir)
    ...
    def _generate_subcategory_stubs(self, category_dir: Path):
        """Generate __init__.pyi stubs for all subcategories recursively using templates."""
        import re
        ...
        for subdir in subdirs:
            ...
            module_aliases = {}    # {alias: actual_module_name}
            ...
            # Find "from . import package" and "from . import package as alias" style imports
            # Pattern captures: from . import client as client_ns -> ('client', 'client_ns')
            # Pattern captures: from . import firewall -> ('firewall', None)
            package_alias_pattern = re.findall(r'from \. import (\w+)(?: as (\w+))?', content)
            for module_name, alias in package_alias_pattern:
                package_imports.append(module_name)
                if alias:  # If there's an alias, map it to the actual module
                    module_aliases[alias] = module_name
            
            # Build endpoints list for template
            endpoints_for_template = [
                {'module_name': module_name, 'class_name': class_name_import}
                for module_name, class_name_import in sorted(set(endpoint_imports))
            ]
            
            # ALSO extract subcategory assignments (self.subcategory = subcategory.SubcategoryClass(client))
            # Pattern matches: self.service = service.Service(client)
            attr_pattern = re.compile(r'self\.(\w+)\s*=\s*(\w+)\.(\w+)\(', re.MULTILINE)
            subcategories_for_template = []
            
            for match in attr_pattern.finditer(content):
                attr_name = match.group(1)
                module_ref = match.group(2)  # This might be an alias
                subcat_class_name = match.group(3)
                
                # Resolve alias to actual module name
                module_name = module_aliases.get(module_ref, module_ref)
                
                # Skip if this is already in endpoints (avoid duplication)
                if any(e['module_name'] == module_name for e in endpoints_for_template):
                    continue
                
                # Check if this subcategory has mode-specific classes by looking at its __init__.pyi
                subcat_dir = subdir / module_name
                subcat_init_pyi = subcat_dir / '__init__.pyi'
                
                has_mode_classes = False
                if subcat_init_pyi.exists():
                    subcat_content = subcat_init_pyi.read_text()
                    # Check if DictMode/ObjectMode classes exist
                    if f'{subcat_class_name}DictMode' in subcat_content and f'{subcat_class_name}ObjectMode' in subcat_content:
                        has_mode_classes = True
                
                subcategories_for_template.append({
                    'attr_name': attr_name,
                    'module_name': module_name,
                    'class_name': subcat_class_name,
                    'has_mode_classes': has_mode_classes,
                })
            
            # Use template to generate stub with mode-specific classes
            if endpoints_for_template or subcategories_for_template:
                # Get category name from subdir name
                category_name = subdir.name
                self.pyi_gen.generate_category_init_stub(
                    category_name,
                    endpoints_for_template,
                    init_pyi,
                    subcategories_for_template  # Pass subcategories to template
                )
                parts = []
                if endpoints_for_template:
                    parts.append(f"{len(endpoints_for_template)} endpoints")
                if subcategories_for_template:
                    parts.append(f"{len(subcategories_for_template)} subcategories")
                print(f"  ✅ Generated {subdir.relative_to(self.output_dir)}/__init__.pyi ({', '.join(parts)})")
            else:
                # Fallback to old method for non-endpoint subcategories (packages)
    ...
    def _generate_toplevel_stubs(self):
        ...
        for category in ['cmdb', 'monitor', 'service']:
            ...
            for match in attr_pattern.finditer(content):
                ...
                # Check if this subcategory has mode-specific classes by looking at its __init__.pyi
                subcat_dir = category_dir / module_name
                subcat_init_pyi = subcat_dir / '__init__.pyi'
                ...
                if subcat_init_pyi.exists():
                    subcat_content = subcat_init_pyi.read_text()
                    # Check if DictMode/ObjectMode classes exist
                    if f'{class_name}DictMode' in subcat_content and f'{class_name}ObjectMode' in subcat_content:
                        has_mode_classes = True
            ...
            # Use template to generate stub with mode-specific classes
            if subcategories_for_template:
                self.pyi_gen.generate_toplevel_init_stub(
                    category,
                    subcategories_for_template,
                    init_pyi
                )
                print(f"  ✅ Generated {category}/__init__.pyi ({len(subcategories_for_template)} subcategories)")
    ...
    def generate_endpoint(self, endpoint_path: str) -> tuple[Path, Path, Path]:
        ...
        endpoint_file = self.endpoint_gen.generate(schema, self.output_dir, schema_data=schema_json)
        ...
        # Generate test - DISABLED (use manual live tests instead)
    ...
    def generate_category(self, category: str) -> list[tuple[Path, Path]]:
        ...
        for schema_file in schema_files:
            ...
            endpoint_file = self.endpoint_gen.generate(schema, self.output_dir, will_have_subdir=will_have_subdir, schema_data=schema_json)
            ...
            # Test generation disabled - use manual live tests
            # workspace_root = self.output_dir.parent.parent.parent.parent.parent.parent
            # test_file = self.test_gen.generate(schema, workspace_root)
            
            results.append((endpoint_file, validator_file, None))
