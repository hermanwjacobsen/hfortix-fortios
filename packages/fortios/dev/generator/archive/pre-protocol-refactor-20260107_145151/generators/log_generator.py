#!/usr/bin/env python3
"""
LOG Endpoint Generator (Improved)
...
This improved generator creates nested classes with:
  - Modern type hints (Union for sync/async support)
  - Better docstrings with parameter documentation
  - FortiObject-style return types in stubs
  - Consistent patterns with other endpoint generators

Example API usage:
  fgt.api.log.disk.ips.archive.get()
  fgt.api.log.disk.event.vpn.get(rows=100, filter='srcip==192.168.1.1')
"""
...
class LogEndpointGenerator:
    """Generator for LOG category endpoints with path parameters (Improved)."""
    
    def __init__(self, swagger_docs_dir: Path, output_dir: Path, stubs_dir: Path | None = None):
        """
    ...
    def _cleanup_old_files(self):
        ...
        # Remove all .py files except __pycache__
        for file in self.log_dir.glob("*.py"):
            if file.name != "__pycache__":
                file.unlink()
                print(f"  🗑️  Removed {file.name}")
        
        # Remove .pyi files only if they're in the main package (shouldn't be there)
        ...
        # Clean up stubs directory if it exists
        if self.stubs_dir:
            stubs_log_dir = self.stubs_dir / "log"
            if stubs_log_dir.exists():
                for file in stubs_log_dir.glob("*.pyi"):
                    file.unlink()
                    print(f"  🗑️  Removed stub {file.name}")
    ...
    def _generate_class_file(self, subcategory: str, structure: Dict[str, Any], paths: Dict[str, Any]) -> str:
        ...
        if root_path in paths:
            ...
            lines.append(f'        ')
            lines.append(f'        Args:')
            lines.append(f'            **kwargs: Query parameters including:')
            lines.append(f'                rows (int): Number of log rows to return')
            lines.append(f'                session_id (int): Session ID for continued retrieval')
            lines.append(f'                serial_no (str): Device serial number')
            lines.append(f'                filter (str): Filter expression(s)')
            lines.append(f'        ')
            lines.append(f'        Returns:')
            lines.append(f'            Dict containing log records')
            lines.append(f'        """')
    ...
    def _add_group_class(self, lines: List[str], parent_class: str, group_name: str, 
        ...
        if 'is_value' in group_data:
            ...
            for endpoint_name, path_template in group_data['endpoints'].items():
                if not endpoint_name or endpoint_name == 'raw':
                    ...
                    lines.append(f'        ')
                    lines.append(f'        Args:')
                    lines.append(f'            **kwargs: Query parameters:')
                    lines.append(f'                rows (int): Number of log rows to return')
                    lines.append(f'                session_id (int): Session ID for continued retrieval')
                    lines.append(f'                filter (str): Filter expression(s)')
                    lines.append(f'        ')
        ...
        if 'param_values' in group_data:
            ...
            for value in group_data['param_values']:
                ...
                lines.append(f'        Args:')
                lines.append(f'            **kwargs: Query parameters:')
                lines.append(f'                rows (int): Number of log rows to return (default: 100)')
                lines.append(f'                session_id (int): Session ID for paginated retrieval')
                lines.append(f'                filter (str): Filter expression (e.g., "srcip==192.168.1.1")')
                lines.append(f'                serial_no (str): Retrieve logs from specific device')
                lines.append(f'        ')
                lines.append(f'        Returns:')
                lines.append(f'            Dict with log records and metadata')
                lines.append(f'        """')
        ...
        if 'is_value' in group_data:
            ...
            for endpoint_name, path_template in group_data['endpoints'].items():
                if endpoint_name and endpoint_name != 'raw':
                    ...
                    lines.append('    def get(self, **kwargs: Any) -> Union[dict[str, Any], Coroutine[Any, Any, dict[str, Any]]]:')
                    lines.append(f'        """')
                    lines.append(f'        Get {endpoint_name} data for this log type.')
                    lines.append(f'        ')
                    lines.append(f'        Args:')
                    lines.append(f'            **kwargs: Additional query parameters')
                    lines.append(f'        ')
                    lines.append(f'        Returns:')
                    lines.append(f'            Dict containing {endpoint_name} data')
                    lines.append(f'        """')
                    lines.append(f'        return self._client.get("log", "/log{path}", **kwargs)')
                    lines.append('')
    ...
    def _generate_init_pyi(self, subcategories: List[str]):
        ...
        # Write to stubs directory if available, otherwise to main package
        if self.stubs_dir:
            init_pyi_file = self.stubs_dir / "log" / "__init__.pyi"
            init_pyi_file.parent.mkdir(parents=True, exist_ok=True)
        else:
    ...
    def _generate_pyi_stubs(self, subcategories: List[str]):
        ...
        for subcat in sorted(subcategories):
            ...
            # Determine where to write .pyi file
            if self.stubs_dir:
                pyi_file = self.stubs_dir / "log" / f"{subcat}.pyi"
