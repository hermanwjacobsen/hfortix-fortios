...
Examples:
  ...
  python generate.py --from-file .dev/endpoints_all.json
    ...
    # Determine stubs directory for separate type stub package
    # Structure: workspace/packages/fortios-stubs/hfortix_fortios-stubs/api/v2/
    stubs_base = base_dir.parent.parent / "packages" / "fortios-stubs" / "hfortix_fortios-stubs"
    stubs_dir = stubs_base / "api" / "v2"
    if stubs_base.exists():
        print(f"ℹ️  Stubs will be written to: {stubs_dir}")
    else:
        stubs_dir = None
        print("ℹ️  Stubs package not found - .pyi files will be inline with .py files")
