SpeedTestSchedule
=================

Configuration endpoint for system/speed_test_schedule.

Python Attribute
----------------

.. code-block:: python

   fgt.api.cmdb.system.speed_test_schedule

Available Methods
-----------------

- ``get()`` - GET operation
- ``post()`` - POST operation
- ``put()`` - PUT operation
- ``delete()`` - DELETE operation

Examples
--------

.. code-block:: python

   from hfortix_fortios import FortiOS
   
   fgt = FortiOS(host='192.168.1.99', token='your-token')

   # List all items
   items = fgt.api.cmdb.system.speed_test_schedule.get()


   # Create new item
   result = fgt.api.cmdb.system.speed_test_schedule.post(
       nkey='value',  # optional
       interface='value',  # optional
       status='value',  # optional
   )

   # Update existing item
   result = fgt.api.cmdb.system.speed_test_schedule.put(
       interface='updated-value',
       status='updated-value',
       diffserv='updated-value',
       server_name='updated-value',
   )

   # Delete item
   result = fgt.api.cmdb.system.speed_test_schedule.delete()


Method Reference
----------------

``get()``
^^^^^^^^^

.. code-block:: python

   get(
       interface=None,
       payload_dict=None,
       attr=None,
       skip_to_datasource=None,
       acs=None,
       search=None,
       vdom=None,
       raw_json=False,
       **kwargs
   )

Select a specific entry from a CLI table.


``post()``
^^^^^^^^^^

.. code-block:: python

   post(
       payload_dict=None,
       nkey=None,
       interface=None,
       status=None,
       diffserv=None,
       server_name=None,
       mode=None,
       schedules=None,
       dynamic_server=None,
       ctrl_port=None,
       server_port=None,
       update_shaper=None,
       update_inbandwidth=None,
       update_outbandwidth=None,
       update_interface_shaping=None,
       # ... more parameters
   )

Create object(s) in this table.


``put()``
^^^^^^^^^

.. code-block:: python

   put(
       interface=None,
       payload_dict=None,
       before=None,
       after=None,
       status=None,
       diffserv=None,
       server_name=None,
       mode=None,
       schedules=None,
       dynamic_server=None,
       ctrl_port=None,
       server_port=None,
       update_shaper=None,
       update_inbandwidth=None,
       update_outbandwidth=None,
       # ... more parameters
   )

Update this specific resource.


``delete()``
^^^^^^^^^^^^

.. code-block:: python

   delete(
       interface=None,
       payload_dict=None,
       vdom=None,
       raw_json=False,
       **kwargs
   )

Delete this specific resource.

