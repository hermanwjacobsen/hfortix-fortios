ExternalIdentityProvider
========================

Configuration endpoint for user/external_identity_provider.

Python Attribute
----------------

.. code-block:: python

   fgt.api.cmdb.user.external_identity_provider

Available Methods
-----------------

- ``get()`` - GET operation
- ``post()`` - POST operation
- ``put()`` - PUT operation
- ``delete()`` - DELETE operation

Examples
--------

.. code-block:: python

   from hfortix_fortios import FortiOS
   
   fgt = FortiOS(host='192.168.1.99', token='your-token')

   # List all items
   items = fgt.api.cmdb.user.external_identity_provider.get()

   # Get specific item by name
   item = fgt.api.cmdb.user.external_identity_provider.get(name='item-name')

   # Create new item
   result = fgt.api.cmdb.user.external_identity_provider.post(
       nkey='value',  # optional
       name='value',  # optional
       type='value',  # optional
   )

   # Update existing item
   result = fgt.api.cmdb.user.external_identity_provider.put(
       name='updated-value',
       type='updated-value',
       version='updated-value',
       url='updated-value',
   )

   # Delete item
   result = fgt.api.cmdb.user.external_identity_provider.delete(name='item-name')


Method Reference
----------------

``get()``
^^^^^^^^^

.. code-block:: python

   get(
       name=None,
       payload_dict=None,
       attr=None,
       skip_to_datasource=None,
       acs=None,
       search=None,
       vdom=None,
       raw_json=False,
       **kwargs
   )

Select a specific entry from a CLI table.


``post()``
^^^^^^^^^^

.. code-block:: python

   post(
       payload_dict=None,
       nkey=None,
       name=None,
       type=None,
       version=None,
       url=None,
       user_attr_name=None,
       group_attr_name=None,
       port=None,
       source_ip=None,
       interface_select_method=None,
       interface=None,
       vrf_select=None,
       server_identity_check=None,
       timeout=None,
       # ... more parameters
   )

Create object(s) in this table.


``put()``
^^^^^^^^^

.. code-block:: python

   put(
       name=None,
       payload_dict=None,
       before=None,
       after=None,
       type=None,
       version=None,
       url=None,
       user_attr_name=None,
       group_attr_name=None,
       port=None,
       source_ip=None,
       interface_select_method=None,
       interface=None,
       vrf_select=None,
       server_identity_check=None,
       # ... more parameters
   )

Update this specific resource.


``delete()``
^^^^^^^^^^^^

.. code-block:: python

   delete(
       name=None,
       payload_dict=None,
       vdom=None,
       raw_json=False,
       **kwargs
   )

Delete this specific resource.

