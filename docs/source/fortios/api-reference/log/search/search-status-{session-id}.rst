.. _search_search_status_{session_id}:

Search_Status_{Session_Id}
==========================

Returns status of log search session, if it is active or not. This is only applicable for disk log search.
...
.. code-block:: python
   ...
   fgt.api.log.search.search_status_{session_id}
...
.. code-block:: python
   ...
   # Query logs
   response = fgt.api.log.search.search_status_{session_id}.get()
